...
class Material:
    def __init__(self, type, composition=None):
        ...
        self.type = type  # 'base', 'double', 'composite', 'indefinido'
    ...
    def _get_text(self):
        if self.type == 'indefinido':
            return "?"
        # For base, double, and composite, join the composition list
...
class Game:
        ...
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                ...
                if material:
                    ...
                    # Determine border color based on material type
                    border_color = BLACK
                    border_width = 2
                    if material.is_selected:
                        border_color = (255, 255, 0) # Yellow for selection
                        border_width = 5
                    elif material.type == 'double':
                        border_color = (0, 0, 200) # Darker Blue for doubles
                        border_width = 3
                    elif material.type == 'composite':
                        border_color = (0, 150, 0) # Darker Green for composites
                        border_width = 3
                    
                    pygame.draw.rect(surface, border_color, rect, border_width, border_radius=10) # Draw the border
    ...
    def set_error_message(self, message, duration=120): # Duration in frames (e.g., 120 frames = 2 seconds at 60 FPS)
    ...
    def try_combine_materials(self):
        ...
        # Rule 3: C1CN... + C1CM... = C1CN...M... (two composite materials with the same root)
            ...
            if mat1.composition[0] == mat2.composition[0]:
                ...
                error_msg = "¡Error! Los materiales compuestos deben tener la misma raíz (primer color) para combinarse."
        else:
            error_msg = "Combinación no válida. Intenta con C1+C1, C1C1+C2C2, o compuestos con la misma raíz."
    ...
    def refill_grid_after_crunch(self, row, col):
        ...
        # Generate a new material in the top row
        ...
        while running:
            ...
            if self.game_state == MENU:
                ...
                tutorial_text = [
                    ...
                    "   - C1 + C1 = C1C1 (ej. Rojo + Rojo = RR). Los materiales dobles tienen un borde azul.",
                    "   - C1C1 + C2C2 = C1C2 (ej. RR + YY = RY). Los materiales compuestos tienen un borde verde.",
                    "   - C1C... + C1C... = C1CC... (ej. RY + RB = RYB). Los materiales compuestos tienen un borde verde.",
