...
import random
import sys
import traceback # Importar el módulo traceback para obtener trazas de error detalladas
...
class Game:
    def __init__(self):
        ...
        self.selected_materials = [] # Lista para almacenar 0, 1 o 2 materiales seleccionados. Inicializado aquí.
    ...
    def add_new_grid(self):
        ...
        # No rellenar aquí, fill_current_grid se llama después de cambiar el índice si se compra una nueva
        # self.fill_current_grid() # Se llama en initialize_game_values y después de cambiar de cuadrícula
    ...
    def handle_game_click(self, pos, button_type):
        ...
        if self.left_arrow_button.handle_event(pygame.event.Event(pygame.MOUSEBUTTONDOWN, {'pos': pos, 'button': button_type})):
            ...
            self.fill_current_grid() # Rellenar la cuadrícula si está vacía o parcialmente vacía
            return
        ...
        if self.buy_grid_button.handle_event(pygame.event.Event(pygame.MOUSEBUTTONDOWN, {'pos': pos, 'button': button_type})):
            if self.current_money >= self.grid_purchase_cost:
                ...
                self.reset_selection()
                self.fill_current_grid() # Rellenar la nueva cuadrícula
            else:
...
if __name__ == "__main__":
    try:
        ...
        # Imprime la traza completa del error para una mejor depuración
        traceback.print_exc()
