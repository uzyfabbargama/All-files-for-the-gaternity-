...
class Quantum:
    ...
    def __init__(self, name, rules):
        """
        rules: Lista de tuplas (N, M) que definen el quantum.
        Ej: [(1, 3), (4, 6)] para '1=3, 4=6'
        """
        self.name = name
    ...
    def get_additive_value(self):
        """
        Calcula el valor aditivo/sustractivo del Quantum (suma de todos los M's).
        """
    ...
    def __mul__(self, other): # Quantum * Real (Escala el Quantum)
        if isinstance(other, (int, float)):
            new_rules = [(n * other, m * other) for n, m in self.rules]
            # Devolvemos un nuevo Quantum. Podríamos pedir un nombre o generar uno.
            # Por simplicidad, le daremos un nombre temporal "Scaled_<original_name>"
            return Quantum(f"Scaled_{self.name}", new_rules)
        return NotImplemented
    ...
    def __truediv__(self, other): # Quantum / Real (Invierte y Multiplica)
        if isinstance(other, (int, float)):
            ...
            new_rules = [(m * other, n * other) for n, m in self.rules] # Invertir (M,N) y multiplicar por other
            return Quantum(f"Div_{self.name}_by_{other}", new_rules)
        return NotImplemented

    def __rtruediv__(self, other): # Real / Quantum (invierte el Quantum y luego se divide el real?)
        # Esto sería A / Q. Aquí la lógica debe ser clara.
        # Por ahora, para simplificar, asumiremos que las operaciones son Quantum OP Real.
        # Si A / Q fuera relevante, podríamos definirlo como un nuevo Quantum con reglas A/N, A/M.
        return NotImplemented # O implementa la lógica específica para A / Q
    ...
    def __floordiv__(self, other): # Usaremos // para la hiperdivisión Q \ A (N/A=M/A)
        if isinstance(other, (int, float)):
            ...
            new_rules = [(n / other, m / other) for n, m in self.rules] # N/A=M/A
            return Quantum(f"HiperDiv_{self.name}_by_{other}", new_rules)
        return NotImplemented
    ...
    def __pow__(self, other): # Quantum ** Real (Invierte y Divide)
        if isinstance(other, (int, float)):
            ...
            new_rules = [(m / other, n / other) for n, m in self.rules] # Invertir (M,N) y dividir por other
            return Quantum(f"HiperMul_{self.name}_by_{other}", new_rules)
        return NotImplemented
...
def calculadora_zeroniana():
    ...
    while True:
        try:
            ...
            # Esto es clave para que eval() reconozca los quantums almacenados
            for q_name, q_obj in Quantum._all_quantums.items():
                processed_input = processed_input.replace(q_name, f"Quantum._all_quantums['{q_name}']")
            
            # Intentar evaluar la expresión completa
            # El dict {'DZ': DZ, 'Quantum': Quantum} le dice a eval que DZ y Quantum son tus clases
            # IMPORTANTE: eval() es potente pero puede ser un riesgo de seguridad si el input no es de confianza.
            # Para este proyecto educativo, está bien.
            ...
            try:
                # Incluimos Quantum en el contexto de eval para que reconozca los objetos Quantum
