...
.saltar_carga:
	...
	add rsi, 500 	; Primera línea (Margen de seguridad)
	mov rcx, 62000	; Contador de celdas por "año"
...
guardar_y_avanzar:
	...
	cmp r12, 1				; ¿Llegamos a los 256 años? (por ahora, año 1)
	je .guardar_eon			; Si, sí, vamos a guardar al disco
	
	;3. Reinicio normal (si no llegó a 10,000)
	mov rsi, universo
	add rsi, 250			; Reset puntero
	mov rcx, 62000			; Reser contador
...
losv:
    ; 1. Intentar crear el Socket (UDP)
    mov rax, 41         
    mov rdi, 2          
    mov rsi, 1 		;Ahora rsi 1         
    mov rdx, 0
    syscall
    mov r13, rax
    cmp rax, 0
    jl .finalizar_red   ; Si falla (RAX < 0), saltamos todo el bloque de red
    

    ; 2. Estructura de dirección en la pila
    push qword 0        
    push 0x0100007f     
    push word 0x3930    
    push word 2         
    mov rsi, rsp         ; R8 apunta a la estructura
	mov rdx, 16
	mov rax, 42
	syscall
    ...
    ; 3. ENVIAR (Syscall 44: sys_sendto)
    mov rax, 44         
    mov rdi, r13        
    mov rsi, universo   
    mov rdx, 62500      
    mov r10, 0 
    mov r8, 0		; En TCP ya está conectado, r8 es 0         
    mov r9, 0          
    syscall
...
.finalizar_red:
    ;--- REINICIO MAESTRO ---
    ; Aquí convergen todos los caminos para empezar el nuevo año
    mov rsi, universo
    add rsi, 500        ; Tu margen de seguridad para 250x250
    mov rcx, 62000      ; Contador ajustado a la nueva escala
    xor r12, r12        ; Reset del contador de eones
    jmp bucle_eterno


section .data
	filename db "universo_short.bin", 0
	timespec:
		dq 0			; segundos
		dq 100000000	; 0 nanosegundos
;--- Fin
fin:
