# =======================================================
# BELLA v2.6: MOTOR DE PERSONALIDAD (4096 NEURONAS)feb. 22, 2026, 19:08:39
# =======================================================
...
def guardar_estado():
    ...
    estado = {
        "memoria": memoria,
        "exps": exps,
        "traductor": traductor # Pickle SI acepta keys como enteros, ¡golazo!
    }
    with open(FILE_NAME, "wb") as f: # "wb" es para escritura binaria
        pickle.dump(estado, f, protocol=pickle.HIGHEST_PROTOCOL)
    
    fin_save = time.perf_counter()
    print(f"\n[SISTEMA]: Conciencia serializada en {fin_save - inicio_save:.2f}s")

def cargar_estado():
    global memoria, exps, traductor
    if os.path.exists(FILE_NAME):
        inicio_load = time.perf_counter()
        with open(FILE_NAME, "rb") as f: # "rb" es para lectura binaria
            estado = pickle.load(f)
            memoria = estado["memoria"]
            exps = estado["exps"]
            traductor = estado["traductor"]
        fin_load = time.perf_counter()
        print(f"[SISTEMA]: Conciencia REANIMADA en {fin_load - inicio_load:.2f}s")
    else:
