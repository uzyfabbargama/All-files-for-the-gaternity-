...
def cargar_xor_pack():
    global memoria, exps
    # Usamos la misma lógica para encontrar el archivo .xrp
    base_name = FILE_NAME.split('.')[0]
    filename_xrp = base_name + ".xrp"
    ...
    if os.path.exists(filename_xrp):
        ...
        # El tamaño de cada paquete es: 4(I) + 1(B) + 8(Q) + 2(H) + 1(B) = 16 bytes
        chunk_size = 16 
        ...
        with open(filename_xrp, "rb") as f:
            while True:
                ...
                # Desempaquetamos siguiendo el mismo orden del guardado
                # > : Big Endian (estándar para redes/binario puro)
                # I : Unsigned Int (ID)
                # B : Unsigned Char (Separador 0x9C)
                # Q : Unsigned Long Long (Memoria)
                # H : Unsigned Short (Interés/Presión)
                # B : Unsigned Char (Separador 0xE8)
                idx, sep1, mem, pres, sep2 = struct.unpack(">IBQHB", chunk)
                
                # Inyectamos los datos en la neurona correspondiente
                if idx < num_n:
                    memoria[idx] = mem
                    exps[idx][2] = pres
                    # Opcional: imprimir un aviso si los separadores están mal (corrupción de datos)
                    if sep1 != 0x9C or sep2 != 0xE8:
                        print(f"[!] Aviso: Posible desalineación en neurona {idx}")

        fin_load = time.perf_counter()
        print(f"[xor pack]: Conciencia REANIMADA en {fin_load - inicio_load:.6f} segundos.")
    else:
        print("[xor pack]: No se encontró archivo binario. Iniciando Tabula Rasa.")        # 1. MOTOR DE EXPERIENCIA (Estabilización Fractal)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() in ["salir", "exit", "quit"]:
            guardar_xor_pack() # <--- GUARDADO ANTES DE MORIR
