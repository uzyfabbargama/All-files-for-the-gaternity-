# =======================================================
# BELLA v3.3: MOTOR DE PERSONALIDAD (4096 NEURONAS)feb. 24, 2026, 11:32:33
# =======================================================
...
# --- PERSISTENCIA v3.3 (XIP + XPR) ---
nombre_usuario = input("Para restaurar sesión (v3.3): ") + ".xpr" # Cambiamos extensión a .xrp
FILE_NAME = os.path.join("sesions", nombre_usuario + ".xpr")
...
def guardar_xor_pack_28():
    ...
    with open(filename_xrp, "wb") as f:
        for idx in activas:
            ...
            paquete = struct.pack(">IBQH", #Eliminamos el largo variable
                                  idx,      # ID (20 bits caben en un Int)
                                  0x9C,     # Tu separador ::
                                  cerebro_asm.mente[idx], # Usando el metal
                                  int(exps[idx][2])) 
    ...
    if os.path.exists(filename_xrp):
        ...
        with open(filename_xrp, "rb") as f:
            while True:
                ...
                header = f.read(14) # Ajustado al nuevo pack fijo
                if not header: break
...
if not cerebro_asm.cargar_mente("bella.xip"):
    print("[XIP]: Memoria virgen detectada.")
    # Inicializamos con el estado base de la ALU
