...
def Numeraso_update():        
            ...
            while caso != 3:
                ...
                #Reemplazamos el sistema del tiempo, para que la lógica sea contraproducente, y agregar otro conflicto al sistema
                ...
                C3 = (Numero_generado // PosC2) % 2 #tercer controlador
                caso = C1 + C2 + C3
            tiempo = expB*10*PosX + expH*20*PosY + expL*30*PosZ
            Fatiga = (Bondad//(10//1))*PosX + (Hostilidad//(10//2))*PosY + (Lógica//(10//3))*PosZ
                
            return Numero_generado % ((base**3) * 2**3), expB, expH, expL, tiempo, Fatiga
...
Numero, expB, expH, expL, tiempo, Fatiga = Numeraso_update() #para acceder al número de la personalidad
...
while True: #si mensaje en exit, salir y guardar
    ...
    try:
            ...
            Necesidad_Social = 1000 - Necesidad_Social #Error solucionado, la IA de eval, decía, "la satisfacción de la necesidad social se ha resuelto en un 99%", lo que leía la IA de la respuesta "tengo una inmensa nevesidad social"
            ...
            Numero1 += Fatiga + tiempo #Cierra el bucle, ahora la lógica es penalizada, y a largo plazo, más difícil.
    ...
    Cagar_actual = max(1,Numero1 // PosX) %base#obtener el estado de la ganas de ir al baño actuales 
    Hambre_actual = max(1,Numero1 // PosY) % base#obtener el estado de la ganas de comer actuales
    Sueño_actual = max(1,Numero1 // PosZ) % base#obtener el estado del sueño actual
    ...
    Escenario_complejo = (
"Eres un diseñador de escenarios." 
f"Toma el siguiente escenario base {Escenario} y expándelo en una descripción detallada de UN párrafo." 
"Toma en cuenta los siguientes valores numéricos para el escenario"
f"Lógica {porL:.2f}%: este es el nivel frío, casi mecánico del escenario, donde se relata como si todo fuera parte de un algoritmo"
f"Bondad {porB:.2f}%: este es el emocional, apasionado, si, bondad, pero también, todo aparenta ser perfecto, ser puro, parece un cuento de hadas, que oculta un trasfondo emocional"
f"Hotilidad {porH:.2f}%: aquí está la cautela, el conflicto interno, discusiones, todo parece estar cerca de un huracán, sin alcanzarlo del todo, como un clímax a punto de suceder"
"Toma y escribe el escenario, teniendo en cuenta el porcentaje más alto, los más bajos colocalos como detalles menores"
)
    ...
    data = { "contents": [ { "parts": [ { "text": Escenario_complejo } ] } ] }
    
    try:
        print(Escenario_complejo)
        response = requests.post(url_ia_response, headers=headers, json=data)
        response.raise_for_status()
        Escenario_completo = response.json()['candidates'][0]['content']['parts'][0]['text']
        print("\033[32m _ \033[0m" * 50)
        print( "\033[42m|_|\033[0m" * 50)
        print(Escenario_completo)
        print("\033[32m _ \033[0m" * 50)
        print( "\033[42m|_|\033[0m" * 50)
        ...
        print(f"Error al procesar la respuesta de la API para generar la respuesta: {e}")
    Resumen = (f"toma el historial del chat {get_prompt_history_text()}\n, y resumelo según lo que dice el usuario en {message}"
    f"Debes resumir lo que consideres importante"
    f"Esto es para evitar la repetición"
    )
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
            api_key = inicio()
    ...
    data = { "contents": [ { "parts": [ { "text": Resumen } ] } ] }
    
    try:
        print(Resumen)
        response = requests.post(url_ia_response, headers=headers, json=data)
        response.raise_for_status()
        Resumen_procesado = response.json()['candidates'][0]['content']['parts'][0]['text']
        print(Resumen_procesado)
    except requests.exceptions.RequestException as e:
        print(f"Error al conectar con la API para generar la respuesta: {e}")
    except (json.JSONDecodeError, KeyError) as e:
        print(f"Error al procesar la respuesta de la API para generar la respuesta: {e}")
    ...
    f"Responde a este resumen de la conversación:\n"
    f"{Resumen_procesado}"
    f"Responde en base a este resumen"
    f"Este es el mensaje del Usuario: '{message}' no tuyo\n\n"
    f"Tu respuesta es la de tu personaje."
    f"Para empezar, toma de punto de partida la descripción de este escenario {Escenario_completo}"
    f"Responde al menos con al menos el doble de la respuesta del usuario"
    f"ATENCIÓN: lógica alta, no significa que debas actuar como robot o máquina, sino con astucia. (lo contrario si es baja)"
    )
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
            api_key = inicio()
    ...
    try:
        ...
        print(f"Error al procesar la respuesta de la API para generar la respuesta: {e}")
#Bloque de Peculiaridad dinámica desactivado.
    # Peculiaridad_dinámica = ( #Toma la peculiaridad actual, la respuesta del usuario, y la respuesta del personaje actual
    #     f"Toma la peculiaridad actual: {Peculiaridad}"
    #     f"Define la peculiaridad del personaje en una o dos frases, tomando en cuenta lo que dijo el usuario en: {message}. Y en lo que dijo el personaje: {respuesta_ia}"
    #     f"¿se resolvió el conflicto?"
    #     )
    ...
    # data = { "contents": [ { "parts": [ { "text": Peculiaridad_dinámica } ] } ] }
    
    # try:
    #     #print(prompt_para_ia)
    #     response = requests.post(url_ia_response, headers=headers, json=data)
    #     response.raise_for_status()
    #     Peculiaridad = response.json()['candidates'][0]['content']['parts'][0]['text']
    # except requests.exceptions.RequestException as e:
    #     print(f"Error al conectar con la API para generar la respuesta: {e}")
    # except (json.JSONDecodeError, KeyError) as e:
    #     print(f"Error al procesar la respuesta de la API para generar la respuesta: {e}")
