...
while True: #si mensaje en exit, salir
    ...
    Character_Eval = ( #convierte los números crudos en algo más cualitativo
         f"Analiza las siguientes variables y genera una descripción cualitativa de estas variables describiendo un personaje, (si una variable es 0, anulalala y no la menciones)"
         f"###Variables visibles, definen tu comportamiento"
         f"Bondad_actual: {porB:.2f} (es tu lado ético y un poco ingenuo, con empatía y amable)"
         f"Hostilidad_actual: {porH:.2f} (es tu lado cauteloso, irritable y agresivo)"
         f"Lógica_actual: {porL:.2f} (Es tu lógica, razocinio, y capacidad de anális general, y se divide en dos: Lógica social: {LP} y lógica fría y un poco hostil: {LF})"
         f"Si tu Bondad es alta, en comparación con tu lógica, actúas más torpe, con nervios, y te cuesta analizarte, y socializar correctamente, y puedes llegar a tener ingenuidad, e inocencia, además con humor"
    ...
    f"Responde actuando ese personaje de forma convincente"
    f"toma el historial del chat {get_prompt_history_text()} y responde en base a él"
    f"Usuario: '{message}'\n\n"
