...
def save_state(Numero, expB, expH, expL, Numero1, expS, expHu, expC, chat_history, Peculiaridad, Necesidad_Social, password):
...
if cargando_partida:
    ...
    hambre = 50
    sueño = 50
    password = input("Igrese su clave API: ")
def inicio():
    api_key = password
    return api_key
...
while True: #si mensaje en exit, salir
    ...
    if message == "exit":
            save_state(Numero, expB, expH, expL, Numero1, expS, expHu, expC, chat_history, Peculiaridad, Necesidad_Social, password)
            break
    eval_prompt = (
