...
DEVUELVE JSON con números 0-1000 (logarítmicos) para emociones y 0-50 para necesidades."""
    ...
    Escenario_complejo = (f"Eres un diseñador de escenarios. Toma el siguiente escenario simple {Escenario} y expándelo en una descripción detallada de un párrafo"
                         f"Describiendo: Objetos y el entorno"
                         f"En base a: {Character} (Es decir toma la personalidad descrita aquí y cambia: No el escenario, sino como se siente)")
    ...
    f"Debes resumir sin quitar matiz emocional del texto"
    f"Esto es para evitar la repetición (pero no te dejes llevar por la lógica)"
    )
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
            api_key = inicio()
