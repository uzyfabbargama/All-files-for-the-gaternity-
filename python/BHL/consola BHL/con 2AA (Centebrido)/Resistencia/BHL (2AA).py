...
while True: #si mensaje en exit, salir
    ...
    try:
            ...
            Necesidad_C = (Numero1 // PorX %100
            Necesidad_H = (Numero1 // PorY %100
            Necesidad_S = Numero1 // PorZ % 100
            Numero -= (Necesidad_C // PorX)%100 + (Necesidad_H // PorX) %100 + (Necesidad_S // PorX)%100 #reduce bondad por las necesidades
            Numero += (Necesidad_C // PorY)%100 + (Necesidad_H // PorY)%100 + (Necesidad_S // PorY)%100 #aumenta la hostilidad ante las necesidades
            Numero -= ((Necesidad_C // PorZ)%100)*2 + ((Necesidad_H // PorZ)%100)*2 + ((Necesidad_S // 10)%100)*2 #disminuye la lógica por 2
    ...
    Bondad_actual = max(1, Numero // PorX %100) #obtener el estado de la bondad actual
    Hostilidad_actual = max(1,Numero // PorY) % 100) #obtener el estado de la hostilidad actual
    Lógica_actual = max(1,(Numero // PorZ) % (100)) #obtener el estado de la lógica actual
    Cagar_actual = max(1,Numero1 // PorX) %100 #obtener el estado de la ganas de ir al baño actuales
    Hambre_actual = max(1,Numero1 // PorY) % 100) #obtener el estado de la ganas de comer actuales
    Sueño_actual = max(1,Numero1 // PorZ) % 100) #obtener el estado del sueño actual
