...
def load_state():
    ...
    return data
#Datos de entrada
a = max(1, int(input("Define la bondad: "))) % x64
b = max(1,int(input("Define la hostilidad: "))) % x64
c = max(1,int(input("Define la Lógica: "))) % x64
...
while True: #si mensaje en exit, salir
    ...
    try:
            ...
            Necesidad_C = (Numero1 // PorX %x64
            Necesidad_H = (Numero1 // PorY %x64
            Necesidad_S = Numero1 // PorZ % x64
            Numero -= (Necesidad_C // PorX) %x64 + (Necesidad_H // PorX) %x64 + (Necesidad_S // PorX)%x64 #reduce bondad por las necesidades
            Numero += (Necesidad_C // PorY)%x64 + (Necesidad_H // PorY)%x64 + (Necesidad_S // PorY)%x64 #aumenta la hostilidad ante las necesidades
            Numero -= ((Necesidad_C // PorZ)%x64)*2 + ((Necesidad_H // PorZ)%x64)*2 + ((Necesidad_S // 10)%x64)*2 #disminuye la lógica por 2
    ...
    Bondad_actual = max(1, Numero // PorX %x64) #obtener el estado de la bondad actual
    Hostilidad_actual = max(1,Numero // PorY) % x64) #obtener el estado de la hostilidad actual
    Lógica_actual = max(1,(Numero // PorZ) % (x64)) #obtener el estado de la lógica actual
    Cagar_actual = max(1,Numero1 // PorX) %x64 #obtener el estado de la ganas de ir al baño actuales
    Hambre_actual = max(1,Numero1 // PorY) % x64) #obtener el estado de la ganas de comer actuales
    Sueño_actual = max(1,Numero1 // PorZ) % x64) #obtener el estado del sueño actual
