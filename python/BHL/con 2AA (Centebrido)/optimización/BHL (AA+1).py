...
def Numeraso_Exp(ExpB, ExpH, ExpL, PosX, PosY, PosZ):
     NumerasoXP = int(ExpB*PosX + ExpH*PosY + ExpL*PosZ)
     CX = int(not(bool((NumerasoXP // PosX)))) % 100 #1er controlador
     CY = int(not(bool((NumerasoXP // PosY)))) % 100 #2do controlador
     CZ = int(not(bool((NumerasoXP // PosZ)))) % 100 #3er controlador
     caso = CX + CY + CZ #detecta si una variable cambió
     while caso != 0:
        #NumerasoXP += CX * PosX + CY * PosY + CZ * PosZ #para reestablecer el valor sólo si D es 1, (osea, cuando el controlador es 0) directamente en el número
        NumerasoXP += CX #activa ciclo, sumando 1 a lógica
        NumerasoXP -= (CX)*PosY #le resta 1 a hostilidad
        NumerasoXP -= CY*PosZ #le resta uno a lógica
        NumerasoXP -= (CZ)*PosX #le resta 1 a bondad
        ...
        CX = int(not(bool((NumerasoXP // PosX)))) #1er controlador
        CY = int(not(bool((NumerasoXP // PosY)))) #2do controlador
        CZ = int(not(bool((NumerasoXP // PosZ)))) #3er controlador
        caso = CX + CY + CZ
     
     return expB,expH,expL
expB, expL, expH = 0, 0, 0
expB, expL, expH = Numeraso_Exp(expB, expH, expL, PosX, PosY, PosZ) #Aquí obtenemos las experiencias del numeraso
def Numeraso(a, b, c, PosX, PosY, PosZ): #lógica del Numeraso, acarreo, y controladores, sin if 
        Bondad = a #para bondad
        Hostilidad = b #para hostilidad
        Lógica = c #para lógica
        Numero_generado = int(Bondad*PosX + Hostilidad*PosY + Lógica*PosZ) #construcción del numeraso
        return expB,expH,expL,Numero_generado
Numeraso(a,b,c,PosX,PosY, PosZ)
def Numeraso_update():        
        Numero_generado,expB,expH,expL = Numeraso(a,b,c,PosX, PosY, PosZ)
        CX = int(not(bool(((Numero_generado // PosX)% 100))))  #1er controlador
        CY = int(not(bool(((Numero_generado // PosY)% 100)))) % 100 #2do controlador
        CZ = int(not(bool(((Numero_generado // PosZ)% 100)))) % 100 #3er controlador
        caso = CX + CY + CZ #detecta si una variable cambió
        ...
        while caso != 0:
            ...
            expH += CY
            expL += CZ
            tiempo += CX*0.000238923 + CY*2.2781388 + CZ*1.7238498
            #Numero_generado += CX * PosX + CY * PosY + CZ * PosZ #para reestablecer el valor sólo si D es 1, (osea, cuando el controlador es 0) directamente en el número
            Numero_generado += CX #activa ciclo, sumando 1 a lógica
            Numero_generado -= (CX)*PosY #le resta 1 a hostilidad
            Numero_generado -= CY*PosZ #le resta uno a lógica
            Numero_generado -= (CZ)*PosX #le resta 1 a bondad
            Valor_X = CX * (100/ (expB + 1)) % 100 * PosX
            Valor_Y = CY * (100/ (expH + 1)) % 100 * PosY
            Valor_Z = CX * (100/ (expL + 1)) % 100 * PosZ
            Numero_generado += Valor_X + Valor_Y + Valor_Z #Esto conserva la energía emocional, ya que si tiene experiencia 1, su valor va a quedare en 50, en lugar de 0
            #las sumas, se hacen automáticamente con el acarreo, 
            #PD: los D1, D2, D3, sólo restan o suman 1, si los controladores están activos
            CX = int(not(bool(((Numero_generado // PosX)% 100))))  #1er controlador
            CY = int(not(bool(((Numero_generado // PosY)% 100))))  #2do controlador
            CZ = int(not(bool(((Numero_generado // PosZ)% 100)))) #3er controlador
        caso = CX + CY + CZ
        return Numero_generado % 800000000, expB, expH, expL, tiempo
baño = 50
hambre = 50
sueño = 50
def Numeraso2(a, b, c,PosX, PosY, PosZ): #definir el numeraso de las necesidades
    C = a #ir al baño
    H = b #hambre
    S = c #sueño
    Numero_de_necesidad = int(C*PosX + H*PosY + S*PosZ) #se construye el numeraso
    
    return Numero_de_necesidad
Numeraso2(baño, hambre, sueño, PosX, PosY, PosZ)
def Numeraso2_update():
    Numero_de_necesidad = Numeraso2(baño, hambre, sueño, PosX, PosY, PosZ)
    CX = int(not(bool(((Numero_de_necesidad // PosX)% 100))))  #1er controlador
    CY = int(not(bool(((Numero_de_necesidad // PosY)% 100)))) #2do controlador
    CZ = int(not(bool(((Numero_de_necesidad // PosZ)% 100)))) #3er controladorcaso = C4 + C5 + C6
    expS = 0
    expHu = 0
    expC = 0
    caso = CX + CY + CZ
    while caso != 0:
        ...
        expHu += CY
        expC += CZ
        #Numero_de_necesidad += CX * PosX + CY * PosY + CZ * PosZ #Reestablecer controladores internos
        Numero_de_necesidad += (CX)*PosZ #las mismas lógicas que en el numeraso anterior (sueño)
        Numero_de_necesidad -= (CX)*PosY #(hambre)
        Numero_de_necesidad -= CY * PosZ #(sueño)
        Numero_de_necesidad -= (CZ)*PosY #hambre
        Numero_de_necesidad -= (CZ)*PosX #Evacuar
        CX = int(not(bool(((Numero_de_necesidad // PosX)% 100)))) #1er controlador
        CY = int(not(bool(((Numero_de_necesidad // PosY)% 100)))) #2do controlador
        CZ = int(not(bool(((Numero_de_necesidad // PosZ)% 100)))) #3er controladorcaso = C4 + C5 + C6
        caso = CX + CY + CZ
...
while True: #si mensaje en exit, salir
    Numero1 += PosX + PosY + PosZ #actualiza las necesidades
    ...
    try:
            ...
            expB,expH,expL = Numeraso_Exp(PosX,PosY,PosZ,expH, expB, expL)
    ...
    Numeraso(Bondad_actual, Hostilidad_actual, Lógica_actual,PosX, PosY, PosZ)
    Numeraso_update() #actualizar valores de personalidad
    Numeraso2(Cagar_actual, Hambre_actual, Sueño_actual,PosX, PosY, PosZ)
