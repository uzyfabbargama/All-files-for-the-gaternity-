...
def evaluar_expresion(expr, contexto):
    """Evalúa expresiones aritméticas y lógicas simples (==, !=, <=, >=, <, >)."""
    # Reemplazo de variables de asignación (orden descendente por longitud para evitar solapamientos)
    for var in sorted(contexto.keys(), key=len, reverse=True):
        expr = expr.replace(var, str(contexto[var]))
    try:
        if re.match(r'^[\d\s\+\-\*\/\(\)\=\!\<\>]+$', expr):
            expr_eval = expr.replace('=', '==').replace('====', '==').replace('!==', '!=')
...
def expandir_lineas_macros(lineas, macros_registradas, definiciones_texto, contexto_assign):
    ...
    while i < len(lineas):
        ...
        # --- MANEJO DE %rep / %endrep (Captura directa de bloque) ---
        ...
        if linea_limpia.startswith('%endrep'):
            ...
            for _ in range(veces_rep):
                # Expandimos pasando el contexto activo directamente
        ...
        # Reemplazamos expresiones %[var] con el estado actual ANTES de evaluar condicionales o instrucciones
        ...
        if linea_limpia.startswith('%if'):
            ...
            if padre_activo and len(partes) > 1:
                condicion_valida = bool(evaluar_expresion(partes[1], contexto_assign))
        ...
        # Si el bloque condicional actual está desactivado, ignoramos
