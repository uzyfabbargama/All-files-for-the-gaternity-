...
def expandir_lineas_macros(lineas, macros_registradas, definiciones_texto, contexto_assign):
    ...
    while i < len(lineas):
        ...
        # Guardamos el texto original para restaurar espacios después
        linea_para_parsear = linea_sin_comentario_lineal.replace('%/ ', '\x00')
        linea_para_parsear = linea_para_parsear.replace(' %/', '\x00')
        linea_para_parsear = linea_para_parsear.replace('%/ ', '\x00')  # Variante con espacio antes
        match_macro = re.match(r'^([a-zA-Z0-9_]+)\s*(.*)$', linea_para_parsear)
        if match_macro:
            ...
            if posible_macro in macros_registradas:
                ...
                # Capturamos tokens entre comillas o sin espacios
                args_llamada = []
                for token in re.findall(r'".+?"|\'.+?\'|\S+', resto_args):
                    # Restauramos espacios donde había %/
                    token = token.replace('\x00', ' ')
                    args_llamada.append(token.strip())
                
                # --- ELIMINAR TOKENS VACÍOS ---
                args_llamada = [a for a in args_llamada if a and a != ',']
