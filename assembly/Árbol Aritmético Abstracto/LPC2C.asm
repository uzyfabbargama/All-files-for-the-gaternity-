...
;Parser[0]
%macro super_if2(2)   ;**
    		mov rdx, 0			;Operacional
            mov rax, %2   		;Resultado éxito
            mov r9, 0    		;Resultado fallo
            ; Check *
            mov dl, [INPUT +rsi];input byte
            shl rdx, 8          ;Preparar para "*"
            ; Check *
            mov dl, [INPUT+rsi+1];
            xor rdx, %1
            cmovnz rax, r9
    ...
    %macro super_if3 (1)
            ...
            mov r9, 0	            ;Registro de Fallo
        ;Check 'V'
            mov dl, [INPUT+rsi]     ;Cargar 'V'
            shl rdx, 8              ;Preparar para "A"
        ;Check 'A'
            mov dl, [INPUT+rsi+1]   ;Cargar 'A'
            shl r9, 8               ;Preparar para "R"
        ;Check 'R'
            mov dl, [INPUT+rsi+2]   ;Cargar 'R'
    ...
    %macro super_if4 (2)
            ...
            mov r9, 0	            ;Registro de Fallo
            ;Check 'C'
            mov dl, [INPUT+rsi] 	; Cargar 'C'
            shl rdx, 8           ;Preparar para "T"
            ;Check 'T'
            mov dl, [INPUT+rsi+1]	; Cargar 'T'
            shl rdx, 8           ;Preparar para "R"
            ; Check 'R'
            mov dl, [INPUT+rsi+2]	; Cargar 'R'
            shl rdx, 8           ;Preparar para "L"
            ; Check 'L'
            mov dl, [INPUT+rsi+3]	; Cargar 'L'
            xor rdx, %1  		; Detectar byte
    ...
    %macro super_if6 (2) ;valor, Pos
            ...
            ;Check 1
            mov dl, [INPUT+rsi]
            shl rdx, 8           ;Preparar para "2"
            ; Check 2 
            mov dl, [INPUT+rsi+1]
            shl rdx, 8           ;Preparar para "3"
            ; Check 3 
            mov dl, [INPUT+rsi+2]  
            shl rdx, 8           ;Preparar para "4"
            ; Check 4 
            mov dl, [INPUT+rsi+3]  
            shl rdx, 8           ;Preparar para "5"
            ; Check 5
            mov dl, [INPUT+rsi+4]  
            shl rdx, 8           ;Preparar para "6"
            ; Check 6
            mov dl, [INPUT+rsi+5]  
            xor rdx, %1
            cmovnz rax, rdx
    ...
    %macro if 2 			;destino, valor
        ...
        mov r9, dl          ;Para comparar
        xor r9, %2          ;INPUT vs valor_real
        mov r9, 0           ;limpieza, sabiendo que mov, no afecta CF
    ...
    %endmacro
    ;usaremos el identado para el r9 scope
    ;R9 scope = R9S
    ;reiniciamos r14
    mov r14, 0
    ;byte identifier
        ...
        super_if3 "VAR", P_VAR
            ...
            skip ;le suma al indice, condicionalmente
        if 1, " " ;le suma 1, si se detecta espacio
            skip      ;recibe 1 para saltear el espacio
                        ...
                        cond(KW_COMMA), r15
                            ...
                            ;if["}"]
                                ...
                                ;if[","]
                                    cond(KW_COMMA), r15
        ;else("a-z,A-Z,0-9")
            elsecond(KW_LLAVES_OPEN), r15
            ;el nombre está en r14
    ;end
    mov r9, 0           ;reinicio contexto
