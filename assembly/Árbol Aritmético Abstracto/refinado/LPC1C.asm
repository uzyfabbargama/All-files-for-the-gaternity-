...
;registros permanentes
    ;rbx PG #se une a R13, para guardar el umbral y valor de cada variable, (hasta 14 bits (de 0 a 14) por cada variable, para el valor, y un último bit para el umbral) = 4 variables por registro
    ;rsp PE
    ;rbp PE
...
;Parser[0]
    ...
    ;Check 'V'
        or r9, INPUT[i]	    ;Cargar 'V'
        shl r9, 8           ;Preparar para "A"
    ;Check 'A'
        or r9, INPUT[i+1]   ;Cargar 'A'
        shl r9, 8           ;Preparar para "R"
    ;Check 'R'
        or r9, INPUT[i+2]   ;Cargar 'R'
        xor r9, %1     	;Si coincide, r9 = 0
        cmovz rax, r10  	; Si falla (r9 != 0), rax = 0.
    %endmacro
    %macro super_if4 (2)
            mov rax, %2          ;Resultado de Éxito
            mov r10, 0	        ;Registro de Fallo
    ...
    %macro detectar (3)
    super_if%1 %2, pos_%3 ;%1 = cantidad de bytes, %2 bytes, %3 valor pos, %4 posición
    do_in r13
