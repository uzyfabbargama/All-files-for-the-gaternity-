...
# Reconstruir los diccionarios con las nuevas claves
...
class NeuronaVisual:
    ...
    def dibujar(self, superficie, is_selected=False, is_dragging=False, is_delete_mode=False, is_rename_mode=False):
        ...
        if current_screen == SCREEN_NETWORK:
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: 
                    ...
                    if eliminar_button_rect.collidepoint(event.pos):
                        ...
                        activation_mode_active = False 
                        modo_conexion_activo = False 
                        modo_renombrar_activo = False # Desactivar otros modos
                        rename_input_active = False
                        selected_neuron_for_rename_visual = None
                        first_clicked_neuron_visual = None
                        connection_message_display_time = 0
                    ...
                    if rename_button_rect.collidepoint(event.pos):
                        modo_renombrar_activo = not modo_renombrar_activo
                        activation_mode_active = False
                        modo_conexion_activo = False
                        modo_eliminar_activo = False
                        # Reset rename state if toggling off
                    ...
                    if plus_button_rect.collidepoint(event.pos):
                        ...
                        input_name_text = "" # Resetear nombre al entrar
                        active_input_name_box = False # Asegurar que el input de nombre no esté activo por defecto
                        input_bits_text = '2' # Mantener '2' como valor por defecto para bits
                        active_input_bits_box = False # Asegurar que el input de bits no esté activo por defecto
                        first_clicked_neuron_visual = None 
                        connection_message_display_time = 0
                        dragging_neuron_visual = None 
                        modo_conexion_activo = False 
                    ...
                    # Lógica de interacción con neuronas (arrastrar/conectar/activar/eliminar/renombrar)
                    ...
                    for visual_neurona in list(neuronas_visuales.values()): 
                        ...
                        if dist_a_neurona <= visual_neurona.radio:
                            ...
                            if modo_renombrar_activo:
                                ...
                                # Posición del input box cerca de la neurona
                                rename_input_rect = pygame.Rect(visual_neurona.x - 75, visual_neurona.y - visual_neurona.radio - 50, 150, 30)
                                set_message(f"Selecciona '{visual_neurona.neurona.id}' para renombrar.")
                                break # Salir del bucle, ya encontramos la neurona
                            ...
                            if modo_eliminar_activo:
                                ...
                                if id_a_borrar in neuronas:
                                    ...
                                    modo_eliminar_activo = False  # Desactivar modo después de seleccionar para eliminar
                            ...
                            # Lógica de CONEXIÓN
                                ...
                                if first_clicked_neuron_visual is None:
                                    ...
                                    first_clicked_neuron_visual = None
                                    connection_message_display_time = 0
                            # Lógica de ARRASTRAR (modo normal)
                            else:
                    ...
                    # Si el clic no fue sobre una neurona y estábamos en modo de conexión o renombrar
                    if not clicked_on_neuron:
                        ...
                        if modo_renombrar_activo and rename_input_active:
                            # Si el clic fue fuera del input box y no en una neurona
            ...
            if event.type == pygame.KEYDOWN:
                if rename_input_active:
                    if event.key == pygame.K_RETURN:
                        ...
                        new_id = rename_input_text.strip() # Eliminar espacios al inicio/final
                        ...
                        if not new_id:
                            ...
                            for neuron_key, _ in neuronas.items():
                                if neuron_key.lower() == new_id.lower() and neuron_key != old_id: # Comparar sin distinguir mayúsculas/minúsculas
                                    is_unique = False
                                    break
                            ...
                            if not is_unique:
                                ...
                                try:
                                    # Actualizar el ID del objeto neurona
                        ...
                        selected_neuron_for_rename_visual = None
                        rename_input_text = ""
                        modo_renombrar_activo = False # Salir del modo renombrar después de confirmar
                    
                    elif event.key == pygame.K_BACKSPACE:
                        rename_input_text = rename_input_text[:-1]
                    else:
                        if len(rename_input_text) < 20: # Limitar longitud del nombre
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: 
                    ...
                    # Comprobar clic en el input de nombre
                    if input_name_rect.collidepoint(event.pos):
                        active_input_name_box = True
                        active_input_bits_box = False # Asegurar que solo un input esté activo
                    # Comprobar clic en el input de bits
                    elif input_bits_rect.collidepoint(event.pos):
                        active_input_bits_box = True
                        active_input_name_box = False # Asegurar que solo un input esté activo
                    else:
                        active_input_name_box = False
                    ...
                    if create_button_rect.collidepoint(event.pos):
                        ...
                        try:
                            ...
                            if not new_id:
                                ...
                                # Comprobar si el ID ya existe (insensible a mayúsculas/minúsculas)
                                is_unique = True
                                ...
                                if not is_unique:
                                    ...
                                    input_name_text = "" # Resetear nombre
                                    input_bits_text = '2' # Resetear bits
                                    current_screen = SCREEN_NETWORK 
            ...
            if event.type == pygame.KEYDOWN:
                if active_input_name_box:
                    if event.key == pygame.K_RETURN:
                        active_input_name_box = False
                        # Podríamos cambiar el foco al campo de bits aquí si lo deseamos
                        active_input_bits_box = True # Cambiar foco al campo de bits
                    elif event.key == pygame.K_BACKSPACE:
                        input_name_text = input_name_text[:-1]
                    else:
                        if len(input_name_text) < 20: # Limitar longitud del nombre
                            input_name_text += event.unicode
                elif active_input_bits_box: # Manejar input para el campo de bits
                    if event.key == pygame.K_RETURN:
                        active_input_bits_box = False
                    elif event.key == pygame.K_BACKSPACE:
                        input_bits_text = input_bits_text[:-1]
                    else:
                        if event.unicode.isdigit() and len(input_bits_text) < 3: # Solo números, limitar a 3 dígitos
    ...
    if current_screen == SCREEN_NETWORK:
        ...
        for visual_neurona in list(neuronas_visuales.values()): 
            if visual_neurona != dragging_neuron_visual:
                ...
                visual_neurona.dibujar(pantalla, is_selected, False, modo_eliminar_activo, is_renaming_this) 
        ...
        if rename_input_active and selected_neuron_for_rename_visual:
            # Re-calcular la posición del input box por si la neurona se movió
            rename_input_rect = pygame.Rect(selected_neuron_for_rename_visual.x - 75, 
                                            selected_neuron_for_rename_visual.y - selected_neuron_for_rename_visual.radio - 50, 
                                            150, 30)

            pygame.draw.rect(pantalla, BLANCO_APAGADO, rename_input_rect, 0, 5)
            pygame.draw.rect(pantalla, AZUL, rename_input_rect, 2, 5) # Borde azul para input activo
        ...
        # Mostrar mensajes temporales (Guardar/Cargar/Errores/Renombrar)
        ...
        try:
            ...
            if preview_bits >= 1:
                ...
                # Ajustar la posición de la vista previa para que no solape los inputs
