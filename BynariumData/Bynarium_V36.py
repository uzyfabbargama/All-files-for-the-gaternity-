...
class NeuronaVisual:
    ...
    def dibujar(self, superficie, zoom, offset_x, offset_y, is_selected=False, is_dragging=False, is_delete_mode=False, is_rename_mode=False):
        ...
        if current_screen == SCREEN_NETWORK:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: # Clic izquierdo
                    # Lógica de botones de control (sin zoom/pan)
                    ...
                    if eliminar_button_rect.collidepoint(event.pos):
                        ...
                        activation_mode_active = False ; modo_conexion_activo = False ; modo_renombrar_activo = False
                        ...
                        activation_mode_active = False ; modo_eliminar_activo = False ; modo_renombrar_activo = False
                        rename_input_active = False; selected_neuron_for_rename_visual = None; first_clicked_neuron_visual = None
                        connection_message_display_time = 0
                        ...
                        activation_mode_active = False; modo_conexion_activo = False; modo_eliminar_activo = False
                        if not modo_renombrar_activo: # Reset rename state if toggling off
                        ...
                        modo_conexion_activo = False ; modo_eliminar_activo = False ; modo_renombrar_activo = False
                        rename_input_active = False; selected_neuron_for_rename_visual = None; first_clicked_neuron_visual = None
                        connection_message_display_time = 0
                        ...
                        set_message(msg, duration=MESSAGE_CRITICAL_DURATION) 
                        if success: # Reset modes on successful load
                            modo_conexion_activo = False; modo_eliminar_activo = False; activation_mode_active = False
                            modo_renombrar_activo = False; rename_input_active = False; selected_neuron_for_rename_visual = None
                        ...
                        activation_mode_active = False; modo_renombrar_activo = False 
                        rename_input_active = False; selected_neuron_for_rename_visual = None
                        set_message("Modo Creador activado.")
                        continue

                    # Lógica de interacción con neuronas (requiere transformación de coordenadas)
                    ...
                    for visual_neurona in list(neuronas_visuales.values()): 
                        ...
                        if dist_a_neurona <= visual_neurona.radio: # Usar radio original para la colisión
                            ...
                            if modo_renombrar_activo:
                                ...
                                # Posición del input box cerca de la neurona (en coordenadas de pantalla)
                                rename_input_rect = pygame.Rect(visual_neurona.x * zoom_level + offset_x * zoom_level - 75, # x transformado
                                                                visual_neurona.y * zoom_level + offset_y * zoom_level - visual_neurona.radio * zoom_level - 50, # y transformado
                                                                150, 30)
                                ...
                                # Modo normal: arrastrar neurona (transformar al espacio de la red)
                ...
                elif event.button == 2: # Clic central del ratón para panning
                    panning_active = True
                ...
                if event.button == 1:
                    if dragging_neuron_visual:
                        dragging_neuron_visual = None
                elif event.button == 2: # Soltar clic central
                    panning_active = False

            elif event.type == pygame.MOUSEMOTION:
                if dragging_neuron_visual:
                    # Transformar la posición del mouse de pantalla a la red
                    transformed_mouse_x = (mouse_x / zoom_level) - offset_x
                    transformed_mouse_y = (mouse_y / zoom_level) - offset_y
                    dragging_neuron_visual.x = transformed_mouse_x - drag_offset_x
                    dragging_neuron_visual.y = transformed_mouse_y - drag_offset_y
                elif panning_active:
                    # Calcular el cambio en el offset
                ...
                # Zoom in/out
                zoom_factor = 1.1 if event.y > 0 else 1 / 1.1 # 1.1 para zoom in, 1/1.1 para zoom out
                
                # Coordenadas del mundo (antes del zoom) donde está el ratón
                world_x_before_zoom = (mouse_x / zoom_level) - offset_x
                world_y_before_zoom = (mouse_y / zoom_level) - offset_y

                zoom_level = max(0.2, min(5.0, zoom_level * zoom_factor)) # Limitar zoom

                # Nuevas coordenadas de offset para mantener el punto del ratón en su lugar
    ...
    if current_screen == SCREEN_NETWORK:
        ...
        for visual_neurona in list(neuronas_visuales.values()): 
            if visual_neurona != dragging_neuron_visual:
                ...
                visual_neurona.dibujar(pantalla, zoom_level, offset_x, offset_y, is_selected, False, modo_eliminar_activo, is_renaming_this) 
        ...
        texto_ayuda_zoom_pan = fuente_pequena.render("Rueda del ratón: Zoom. Botón central: Pan.", True, NEGRO)
        pantalla.blit(texto_ayuda_zoom_pan, (20, texto_ayuda_y_offset + len(texto_ayuda_teclas) * 15 + 50))


        # Mensaje de conexión (sin zoom/pan)
