...
NEURON_DESCRIPTIONS = {
    ...
    "NM": "Memoria",
    "MC": "Input Minecraft" # Nueva descripción para neuronas Minecraft
}
...
class NeuronaVisual:
    ...
    # --- NUEVO MÉTODO: trigger_flash ---
    def trigger_flash(self):
        self.flash_start_time = time.time()
    # --- FIN NUEVO MÉTODO ---

    def dibujar(self, superficie, zoom, offset_x, offset_y, is_selected=False, is_dragging=False, is_delete_mode=False, is_rename_mode=False, is_disconnect_mode=False, is_minecraft_mode=False):
        ...
        if is_delete_mode:
            ...
            display_color = CIAN
        elif is_minecraft_mode and self.neurona.tipo == "MC": # Resaltar neuronas MC en modo MC
            display_color = AMARILLO
...
def inicializar_red_predeterminada():
    ...
    # New Minecraft Input Neurons
    n_mc_fw = NeuronaBynarium("MC_FW", 2)
    n_mc_bw = NeuronaBynarium("MC_BW", 2)
    n_mc_lt = NeuronaBynarium("MC_LT", 2)
    n_mc_rt = NeuronaBynarium("MC_RT", 2)
    n_mc_jmp = NeuronaBynarium("MC_JMP", 2)
    n_mc_sprt = NeuronaBynarium("MC_SPRT", 2)

    neuronas.update({
        "MC_FW": n_mc_fw, "MC_BW": n_mc_bw, "MC_LT": n_mc_lt,
        "MC_RT": n_mc_rt, "MC_JMP": n_mc_jmp, "MC_SPRT": n_mc_sprt
    })

    # Positions for Minecraft neurons (bottom center, distinct from main network)
    mc_start_x = ANCHO_PANTALLA // 2 - 170
    mc_start_y = ALTO_PANTALLA - 150
    mc_offset_x = 70
    mc_offset_y = 50
    
    mc_positions = {
        "MC_FW": (mc_start_x, mc_start_y),
        "MC_BW": (mc_start_x + mc_offset_x, mc_start_y),
        "MC_LT": (mc_start_x + 2 * mc_offset_x, mc_start_y),
        "MC_RT": (mc_start_x + 3 * mc_offset_x, mc_start_y),
        "MC_JMP": (mc_start_x + mc_offset_x * 1.5, mc_start_y + mc_offset_y),
        "MC_SPRT": (mc_start_x + mc_offset_x * 2.5, mc_start_y + mc_offset_y)
    }

    for id, pos in mc_positions.items():
...
modo_minecraft_activo = False # Nuevo: Modo de Sincronización Minecraft
...
while running:
        ...
        if current_screen == SCREEN_NETWORK:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1: # Clic izquierdo
                    ...
                    minecraft_button_rect = pygame.Rect(ANCHO_PANTALLA - 380, ALTO_PANTALLA - 70, 50, 50) # Nuevo botón Minecraft
                    activate_nodes_button_rect = pygame.Rect(ANCHO_PANTALLA - 480, ALTO_PANTALLA - 70, 100, 50) # Ajustar posición
                    save_button_rect = pygame.Rect(ANCHO_PANTALLA - 540, ALTO_PANTALLA - 70, 50, 50) # Ajustar posición
                    load_button_rect = pygame.Rect(ANCHO_PANTALLA - 600, ALTO_PANTALLA - 70, 50, 50) # Ajustar posición
                    ...
                    if eliminar_button_rect.collidepoint(event.pos):
                        ...
                        activation_mode_active = False ; modo_conexion_activo = False ; modo_renombrar_activo = False; modo_desconectar_activo = False; modo_minecraft_activo = False
                        ...
                        activation_mode_active = False ; modo_eliminar_activo = False ; modo_renombrar_activo = False; modo_desconectar_activo = False; modo_minecraft_activo = False
                        rename_input_active = False; selected_neuron_for_rename_visual = None; first_clicked_neuron_visual = None
                        connection_message_display_time = 0
                        ...
                        activation_mode_active = False; modo_conexion_activo = False; modo_eliminar_activo = False; modo_desconectar_activo = False; modo_minecraft_activo = False
                        ...
                        activation_mode_active = False; modo_conexion_activo = False; modo_eliminar_activo = False; modo_renombrar_activo = False; modo_minecraft_activo = False
                        first_clicked_neuron_visual = None 
                        set_message(f"Modo desconectar {'activado' if modo_desconectar_activo else 'desactivado'}.")
                        continue
                    elif minecraft_button_rect.collidepoint(event.pos): # Nuevo: Lógica del botón Minecraft
                        modo_minecraft_activo = not modo_minecraft_activo
                        activation_mode_active = False; modo_conexion_activo = False; modo_eliminar_activo = False; modo_renombrar_activo = False; modo_desconectar_activo = False
                        set_message(f"Modo Minecraft {'activado' if modo_minecraft_activo else 'desactivado'}.")
                        continue
                    elif activate_nodes_button_rect.collidepoint(event.pos):
                        activation_mode_active = not activation_mode_active
                        modo_conexion_activo = False ; modo_eliminar_activo = False ; modo_renombrar_activo = False; modo_desconectar_activo = False; modo_minecraft_activo = False
                        rename_input_active = False; selected_neuron_for_rename_visual = None; first_clicked_neuron_visual = None
                        connection_message_display_time = 0
                        ...
                        if success: 
                            ...
                            modo_renombrar_activo = False; modo_desconectar_activo = False; modo_minecraft_activo = False
                        ...
                        activation_mode_active = False; modo_renombrar_activo = False ; modo_desconectar_activo = False; modo_minecraft_activo = False
                        rename_input_active = False; selected_neuron_for_rename_visual = None
                        ...
                        if dist_a_neurona <= visual_neurona.radio: 
                            ...
                            if modo_renombrar_activo:
                                ...
                                if first_clicked_neuron_visual is None:
                                    ...
                                    elif origen_desc in destino_desc.conectada_a: 
                    ...
                    if not clicked_on_neuron:
                        ...
                        if not modo_eliminar_activo and not modo_desconectar_activo and \
                           ...
                           not modo_renombrar_activo and not dragging_neuron_visual and not modo_minecraft_activo: # No activar panning si modo Minecraft está activo
                            panning_active = True
            ...
            # Manejo de teclado para renombrar o Minecraft o Bynarium
            if event.type == pygame.KEYDOWN:
                if rename_input_active: # Prioridad 1: Input de renombrado
                ...
                # Prioridad 2: Modo Minecraft (si está activo)
                elif modo_minecraft_activo:
                    activated_mc_neuron = None
                    if event.key == pygame.K_w:
                        activated_mc_neuron = neuronas.get("MC_FW")
                        set_message("Minecraft: Adelante (W)", duration=0.8)
                    elif event.key == pygame.K_s:
                        activated_mc_neuron = neuronas.get("MC_BW")
                        set_message("Minecraft: Atrás (S)", duration=0.8)
                    elif event.key == pygame.K_a:
                        activated_mc_neuron = neuronas.get("MC_LT")
                        set_message("Minecraft: Izquierda (A)", duration=0.8)
                    elif event.key == pygame.K_d:
                        activated_mc_neuron = neuronas.get("MC_RT")
                        set_message("Minecraft: Derecha (D)", duration=0.8)
                    elif event.key == pygame.K_SPACE:
                        activated_mc_neuron = neuronas.get("MC_JMP")
                        set_message("Minecraft: Saltar (Espacio)", duration=0.8)
                    elif event.key == pygame.K_LSHIFT or event.key == pygame.K_RSHIFT:
                        activated_mc_neuron = neuronas.get("MC_SPRT")
                        set_message("Minecraft: Sprint/Sigilo (Shift)", duration=0.8)
                    
                    if activated_mc_neuron:
                        if activated_mc_neuron.recibir_activacion():
                            neuronas_visuales[activated_mc_neuron.id].trigger_flash()
                        else:
                            set_message(f"MC: '{activated_mc_neuron.id}' ya está al máximo.", duration=1.5)

                # Prioridad 3: Otros atajos de teclado (si no está en modo renombrar ni Minecraft)
                elif not rename_input_active and not modo_minecraft_activo:
                    ...
                    if event.key == pygame.K_SPACE: # Re-activar NS de nivel 2 si no está en modo MC
                        set_message("Activando neuronas de nivel 2...", duration=1) 
                        for id, neurona_obj in list(neuronas.items()): 
                            if neurona_obj.bits == 2 and neurona_obj.tipo != "MC": # NS, pero no las de Minecraft
                    ...
                    if event.key == pygame.K_a: # Re-activar aleatoria si no está en modo MC
                        if neuronas: 
    ...
    if current_screen == SCREEN_NETWORK:
        ...
        minecraft_button_rect = pygame.Rect(ANCHO_PANTALLA - 380, button_y, button_width, button_height) # Botón Minecraft
        activate_nodes_button_rect = pygame.Rect(ANCHO_PANTALLA - 480, button_y, 100, button_height) 
        save_button_rect = pygame.Rect(ANCHO_PANTALLA - 540, button_y, button_width, button_height) 
        load_button_rect = pygame.Rect(ANCHO_PANTALLA - 600, button_y, button_width, button_height) 
        ...
        # Dibujar botón "🎮" (Minecraft)
        color_minecraft_button = AMARILLO if modo_minecraft_activo else GRIS
        pygame.draw.rect(pantalla, color_minecraft_button, minecraft_button_rect, 0, 5)
        pygame.draw.rect(pantalla, NEGRO, minecraft_button_rect, 2, 5)
        texto_minecraft = fuente.render("🎮", True, NEGRO)
        pantalla.blit(texto_minecraft, (minecraft_button_rect.x + 10, minecraft_button_rect.y + 8))
        ...
        for visual_neurona in list(neuronas_visuales.values()): 
            if visual_neurona != dragging_neuron_visual:
                ...
                visual_neurona.dibujar(pantalla, zoom_level, offset_x, offset_y, is_selected, False, modo_eliminar_activo, is_renaming_this, is_disconnecting_this, modo_minecraft_activo) 
        ...
        if dragging_neuron_visual:
            dragging_neuron_visual.dibujar(pantalla, zoom_level, offset_x, offset_y, False, True, modo_eliminar_activo, False, False, modo_minecraft_activo) 
        ...
        texto_ayuda_y_start = 20
        for i, linea in enumerate(texto_ayuda_teclas):
            texto_render = fuente_pequena.render(linea, True, NEGRO)
            pantalla.blit(texto_render, (20, texto_ayuda_y_start + i * 15))
        
        # Ajustar la posición de las instrucciones de ayuda para el nuevo contenido
        current_y = texto_ayuda_y_start + len(texto_ayuda_teclas) * 15 + 5

        texto_ayuda_drag = fuente_pequena.render("Clic y arrastrar para mover neuronas.", True, NEGRO)
        pantalla.blit(texto_ayuda_drag, (20, current_y)); current_y += 15
        ...
        pantalla.blit(texto_ayuda_disconnect, (20, current_y)); current_y += 15

        texto_ayuda_zoom_pan = fuente_pequena.render("Rueda del ratón: Zoom. Clic IZQ (fuera de neurona): Pan. Clic MEDIO: Pan.", True, NEGRO)
        pantalla.blit(texto_ayuda_zoom_pan, (20, current_y)); current_y += 15

        texto_ayuda_minecraft = fuente_pequena.render("Modo Minecraft (🎮): Teclas WASD/ESPACIO/SHIFT activan neuronas MC.", True, NEGRO)
        pantalla.blit(texto_ayuda_minecraft, (20, current_y)); current_y += 15
