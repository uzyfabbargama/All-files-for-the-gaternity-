...
import math
import json
import os # Importar el módulo os para manejar rutas de archivos y directorios
...
# Definir la ruta de la carpeta de guardado (dentro del directorio del script)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_DIR = os.path.join(SCRIPT_DIR, "bynarium_saves")
SAVE_FILENAME = "red_bynarium.json"
FULL_SAVE_PATH = os.path.join(SAVE_DIR, SAVE_FILENAME)

# --- DEBUGGING INICIO ---
print(f"DEBUG: SCRIPT_DIR: {SCRIPT_DIR}")
print(f"DEBUG: CURRENT_WORKING_DIR: {os.getcwd()}") # Nueva línea de depuración
...
def guardar_red():
    ...
    try:
        ...
        print(f"DEBUG: Directorio '{SAVE_DIR}' asegurado/creado.")
    except Exception as e:
        print(f"ERROR: Falló al crear el directorio '{SAVE_DIR}': {e}")
        return False, f"Error: No se pudo crear la carpeta de guardado. {e}"
    ...
    try:
        print(f"Intentando guardar la red en '{FULL_SAVE_PATH}'...")
        with open(FULL_SAVE_PATH, 'w') as f:
            json.dump(data_to_save, f, indent=4)
        print(f"Éxito: Red guardada en '{FULL_SAVE_PATH}'.")
        return True, f"Red guardada en '{os.path.basename(SAVE_FILENAME)}'." # Mensaje más corto para la UI
    except Exception as e:
        print(f"ERROR: Falló al guardar la red: {e}") 
...
def cargar_red():
    ...
    try:
        print(f"Intentando cargar la red desde '{FULL_SAVE_PATH}'...")
