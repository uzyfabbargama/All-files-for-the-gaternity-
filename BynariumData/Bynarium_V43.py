            ...
            if event.type == pygame.KEYDOWN:
                ...
                if not rename_input_active:
                    ...
                    if event.key == pygame.K_c:
                        ...
                        # --- FASE 1: PREPARACIÓN Y RECOLECCIÓN DE NEURONAS PROPAGADORAS ---
                        neurons_to_propagate_this_cycle = []
                        
                        # Reset input counters for Rol 1 neurons for this cycle
                        # This MUST happen BEFORE any propagation logic that calls recibir_activacion
                        for n in neuronas.values():
                            if n.bits == 1:
                                n.inputs_recibidos_este_ciclo = 0 # <--- AHORA ESTO ESTÁ BIEN UBICADO

                        # Collect neurons ready to propagate (based on their state *at the start of this cycle*)
                        for n in neuronas.values():
                        ...
                        # (Este bloque se mantiene igual, ya que opera sobre el estado de las neuronas y no interfiere con el reseteo de inputs_recibidos_este_ciclo)
                        GOVERNOR_THRESHOLD_BITS = 10 
                        VULNERABLE_VU_THRESHOLD = 5 
                        VULNERABLE_HS_THRESHOLD_FACTOR = 0.5 
                        ...
                        for governor in governors_ready_to_heal:
                            ...
                            for target_n in neuronas.values():
                                if target_n.id == governor.id: continue
                            ...
                            if vulnerable_neurons:
                                ...
                                if not governor.conectar(selected_vulnerable):
                                    pass
                                selected_vulnerable.conectar(governor)
                            ...
                            if neurona_origen.bits == 1: # Rol 1 (NOT) propagation
                                if neurona_origen.activacion == 0: # Rol 1 is in active state (no input), generates '1'
                                    for destino_neurona in potential_destinations:
                                        # Rol 1 'generates' 1, so it calls recibir_activacion with 1
                                        if destino_neurona.recibir_activacion(1): 
                                            ...
                                            propagated_any = True
                                # After a Rol 1 propagates (or not), its *final* activacion state for the *next* cycle
                                # will be determined by whether it received any inputs *in this current cycle*.
                                # This update will happen in a separate loop *after* all propagations.
                            else: # Normal neuron propagation
                                initial_activation = neurona_origen.activacion
                                for destino_neurona in potential_destinations:
                                    if neurona_origen.activacion > 0: 
                                        # Only propagate if destination can receive AND is not full
                                        if destino_neurona.bits == 1: # Destination is a Rol 1
                                            # Special handling: Rol 1 destinations just register they received input
                                            if destino_neurona.recibir_activacion(1): # Call recibir_activacion with 1
                                                neuronas_visuales[destino_neurona.id].trigger_flash()
                                                set_message(f"Propagation: {neurona_origen.id} -> {destino_neurona.id}", duration=0.5)
                                                neurona_origen.activacion -= 1
                                                propagated_any = True
                                        elif destino_neurona.activacion < destino_neurona.max_almacenamiento: # Normal neuron destination
                                            if destino_neurona.recibir_activacion(1): 
                                                neuronas_visuales[destino_neurona.id].trigger_flash()
                                                set_message(f"Propagation: {neurona_origen.id} -> {destino_neurona.id}", duration=0.5)
                                                neurona_origen.activacion -= 1
                                ...
                                # Reset normal neuron activacion after propagation
                                neurona_origen.activacion = 0
                        
                        # --- FASE 3: ACTUALIZACIÓN DE ESTADO PARA ROL 1 (DESPUÉS DE TODAS LAS RECEPCIONES) ---
                        for neurona_obj in neuronas.values():
                            if neurona_obj.bits == 1:
                                if neurona_obj.inputs_recibidos_este_ciclo > 0:
                                    neurona_obj.activacion = 1 # If received any input, it's inactive (doesn't generate)
                                else:
                                    neurona_obj.activacion = 0 # If received NO input, it's active (generates 1)
