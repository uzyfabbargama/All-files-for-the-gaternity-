import sys
from ursina import Ursina, Entity, color, Button, scene, camera, application
...
def generate_numerium_chunk(seed, chunk_x, chunk_z, chunk_size_x, chunk_size_z, chunk_max_y):
    ...
    current_sequence_value = seed + (chunk_x * 100000) + (chunk_z * 1000) 
    ...
    # Aquí es donde Numerium es eficiente: solo sumas y módulos.
    # El valor 1000 es arbitrario, puedes ajustarlo para ver mundos más o menos "llenos".
    ...
    for _ in range(num_filling_iterations):
        ...
        x_local, z_local = map_value_to_coords(current_sequence_value, 3, 3) 
        ...
        x_in_chunk = x_local % chunk_size_x
        z_in_chunk = z_local % chunk_size_z
        ...
        chunk_heights[(x_in_chunk, z_in_chunk)] = min(current_y + 1, chunk_max_y - 1) # Limitamos la altura máxima
...
GLOBAL_SEED = 12345 # Semilla global para el mundo Randomixer/Numerium
CHUNK_SIZE = 16    # Tamaño de cada chunk (ej. 16x16 bloques)
RENDER_DISTANCE = 4 # Cuántos chunks alrededor del jugador deben estar cargados
MAX_Y_HEIGHT = 64 # Altura máxima posible para el terreno en un chunk
...
# --- Voxel para Interacción (para saber qué bloque estamos "apuntando") ---
class Voxel(Button):
    def __init__(self, position=(0,0,0), input_color=color.white):
        super().__init__(
            ...
            texture='white_cube', # Una textura simple
            color=input_color,
    ...
    def input(self, key):
        if self.hovered: # Si el mouse está sobre este voxel
            if key == 'left mouse down': # Clic izquierdo para destruir
                ...
                # Aquí se podría actualizar una matriz de voxeles en memoria para el mundo
            if key == 'right mouse down': # Clic derecho para colocar
                # Coloca un nuevo bloque encima del que estamos apuntando
                new_voxel_pos = self.position + mouse.normal # mouse.normal es la cara a la que apuntamos
                Voxel(position=new_voxel_pos, input_color=color.dark_gray) # Nuevo bloque gris
...
def unload_chunk(cx, cz):
    ...
    print(f"Descargando chunk: ({cx}, {cz})")
...
def update_chunks():
    current_player_chunk_x = math.floor(player.x / CHUNK_SIZE)
    current_player_chunk_z = math.floor(player.z / CHUNK_SIZE)
...
Entity(model='cube', scale=(2,1,2), position=(0,-1,0), color=color.gray, collider='box', texture='white_cube')


# Llamada inicial para cargar los chunks alrededor del punto de inicio del jugador
update_chunks() 

# --- Bucle de Actualización del Juego (llamado cada frame) ---
# En Ursina, la función `update` se llama automáticamente cada frame.
last_player_chunk_x = math.floor(player.x / CHUNK_SIZE)
last_player_chunk_z = math.floor(player.z / CHUNK_SIZE)

def update():
    global last_player_chunk_x, last_player_chunk_z
    
    # Actualiza los chunks solo si el jugador se ha movido a un nuevo chunk
    current_player_chunk_x = math.floor(player.x / CHUNK_SIZE)
    current_player_chunk_z = math.floor(player.z / CHUNK_SIZE)

    if current_player_chunk_x != last_player_chunk_x or \
       current_player_chunk_z != last_player_chunk_z:
        update_chunks()
        last_player_chunk_x = current_player_chunk_x
        last_player_chunk_z = current_player_chunk_z
...
application.development_mode = False # Desactiva el modo desarrollo para evitar reinicios automáticos
application.base.win.set_close_callback(on_quit) # Configura la función de cierre
