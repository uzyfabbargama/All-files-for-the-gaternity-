...
def get_numeraso(bhl_values, chs_values, per, chosen_type_chs):
    """
    Genera el NUMERASO con la lógica de acarreo de PSeInt (80+).
    """
    ...
    # --- Lógica de 80+ para crear el numeraso (el "bus" de datos) ---
    numeraso_bhl = 0
    for i, char in enumerate(per):
        var_name = per_map[char]
        var_value = bhl_values[var_name]
        # Se usa 80 + el valor para crear el "bus" de datos con acarreo
    ...
    # Esto detecta cuándo el valor original del numeraso llega a 99 (9 en el bus de datos),
    # indicando un desborde que afecta a la siguiente variable.
    
    # Controlador para la primera variable (BHL)
    dc = math.trunc(full_numeraso / (10**C9)) % 10
    if dc == 9: # Si el digito de control es 9, significa que hubo un acarreo
