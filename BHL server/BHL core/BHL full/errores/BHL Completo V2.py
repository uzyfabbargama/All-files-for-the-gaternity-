...
def save_state(bhl_values, chs_values, chat_history, per, chosen_type_chs, shadow_values):
    """Guarda el estado actual del personaje y la conversación en un archivo JSON, incluyendo las variables de sombra."""
    data = {
...
def load_state():
    """Carga el estado del personaje y la conversación desde un archivo JSON, incluyendo las variables de sombra."""
...
def update_shadow_desire_and_ego(bhl_values, shadow_values):
    ...
    # Un deseo alto significa que el rasgo ha sido reprimido y luego la personalidad principal cambió,
    # creando una tensión.
    if sb_prev > 0:
    ...
    # El ego aumenta cuando la personalidad principal es alta y la sombra es baja (poca represión).
    # Disminuye cuando hay un conflicto interno (deseo alto).
    ...
    # Nos aseguramos de que todos los valores se mantengan en el rango [0, 100]
...
def bhl_chs_algorithm():
    ...
    f"- **Egoísmo:** {porEgo:.2f}%\n"
    f"Además, tu sombra (los rasgos que reprimes) se manifiestan como:\n"
    f"- **Deseo de Bondad (db):** {shadow_values['db']:.2f}% (Alta cuando te vuelves hostil y añoras ser bueno)\n"
    f"- **Deseo de Hostilidad (dh):** {shadow_values['dh']:.2f}% (Alta cuando te vuelves bueno y añoras ser más firme)\n"
    f"- **Deseo de Lógica (dl):** {shadow_values['dl']:.2f}% (Alta cuando te vuelves emocional y añoras ser objetivo)\n"
    f"Tu 'numeraso' (la interacción emergente de tus rasgos) es: {numeraso}\n"
    ...
    f"- **Lujuria:** {variables_chs['lujuria']}\n"
