...
def bhl_chs_algorithm():
    ...
    if not loaded_data:
        ...
        while True:
            try:
                ...
                print("Error, ingrese nuevamente")
        # [Se omiten las demás preguntas de BHL por brevedad, asumiendo que el usuario las completará]
        while True:
            try:
    ...
    while True:
        # Mostrar el estado actual de los sistemas
        print("\n" + "="*70)
        ...
        # --- Lógica de actualización de estado CHS por tiempo ---
        update_chs_state(variables_chs, chosen_type_chs)
        
        # --- Lógica de actualización de estado CHS por palabras clave del usuario ---
        ...
        # Asegurarse de que los valores CHS no superen los límites
        for key in variables_chs:
            variables_chs[key] = max(0, min(99, int(variables_chs[key])))

        # --- Lógica de calificación del usuario (sin cambios) ---
        eval_prompt = (
        ...
        # Actualización de valores BHL con la calificación automática
        b += ab
        h += ah
        l += al
        s += a_soberbia
        ...
        # El prompt combinado y mejorado para la IA
        prompt_history_text = "\n".join([f"Usuario: {entry['user']}\nPersonaje: {entry['character']}" for entry in chat_history])
        
        # --- Nuevo: Incluye las necesidades biológicas en el prompt ---
        prompt_para_ia = (
        ...
        # El resto del código de la API de Gemini es el mismo que en el script original
        api_key = os.getenv("GEMINI_API_KEY")
        if not api_key:
            api_key = "AIzaSyBa6T3dKbfNA_SxI_dj5Ymk7fB6gk8i3zM" # <-- PEGA TU CLAVE DE API AQUÍ
