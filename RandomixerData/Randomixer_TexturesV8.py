...
BLOCK_SIZE = 32
FPS = 60
TEXTURE_FOLDER = "textures"  # Donde guardás tus .png
TEXTURE_SIZE = BLOCK_SIZE

texture_cache = {}

def load_and_tint_texture(file_name, tint_color):
    key = (file_name, tint_color)
    if key not in texture_cache:
        try:
            path = os.path.join(TEXTURE_FOLDER, file_name)
            base = pygame.image.load(path).convert_alpha()
            tinted = base.copy()
            tinted.fill(tint_color + (0,), special_flags=pygame.BLEND_MULT)
            texture_cache[key] = tinted
        except Exception as e:
            print(f"Error al cargar o tintar {file_name}: {e}")
            fallback = pygame.Surface((TEXTURE_SIZE, TEXTURE_SIZE))
            fallback.fill((255, 0, 255))  # Magenta de error
            texture_cache[key] = fallback
    return texture_cache[key]
with open("texture_file.json") as f:
    Texture_file = json.load(f)
def material_lookup(material_id):
    mapping = {
        1: "Agua",
        2: "Piedra",
        3: "Madera",
    ...
    return mapping.get(material_id, "Piedra")
def material_lookup(material_id):
    mapping = {
        1: "Agua",
        2: "Piedra",
    ...
    return mapping.get(material_id, "Piedra")  # "Piedra" como default
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, world_blocks, world_generator): 
        ...
        for block in world_blocks: # world_blocks son los bloques activos actualmente en el render
            if temp_rect.colliderect(block.rect) and not block.is_fluid():
                ...
                temp_rect.x = self.world_x # Actualizar rect temporal

        # --- Gravity and Vertical Movement (en coordenadas de mundo) ---
        # La gravedad se aplica como una aceleración por cuadro
        self.vel_y += self.gravity_force / FPS / 10 # Se mantiene el /10 para un control más fino
        self.world_y += self.vel_y

        temp_rect.y = self.world_y # Actualizar rect temporal para Y

        for block in world_blocks:
            if temp_rect.colliderect(block.rect) and not block.is_fluid():
                if self.vel_y > 0: # Cayendo, colisionó con el techo de un bloque
                    self.world_y = block.rect.top - BLOCK_SIZE * 2
                    self.vel_y = 0
                    self.on_ground = True
                elif self.vel_y < 0: # Saltando, colisionó con la base de un bloque
                    self.world_y = block.rect.bottom
                    self.vel_y = 0
                temp_rect.y = self.world_y # Actualizar rect temporal

        # No hay límite inferior de pantalla, el mundo continúa hacia abajo (caerías al vacío)
        # Puedes añadir un límite si quieres que el jugador "muera" al caer muy lejos
        # if self.world_y > world_generator.max_world_y_limit:
        #    # Implementar lógica de muerte o respawn
    ...
    def jump(self, current_gravity):
        """
        Calcula la velocidad inicial necesaria para saltar la altura deseada (self.jump_force_factor).
        self.jump_force_factor ahora es la ALTURA en "metros" conceptuales (bloques).
        """
        if self.on_ground:
            # Altura deseada en píxeles
            desired_height_pixels = self.jump_force_factor * BLOCK_SIZE
            
            # Asegurarse de que la gravedad tenga un valor mínimo para evitar división por cero
            # o saltos infinitos con gravedad cero.
            effective_gravity = max(0.1, current_gravity) # Usar la gravedad del universo
            
            # La aceleración de la gravedad por cuadro en píxeles/cuadro^2
            gravity_acceleration_per_frame = effective_gravity / FPS / 10 
            
            # Fórmula para la velocidad inicial (v0 = sqrt(2 * a * h))
            # Ajustamos para evitar sqrt de números negativos si desired_height_pixels es 0 o muy pequeño
            ...
            self.on_ground = False # Ya no está en el suelo una vez que salta
...
class Block(pygame.sprite.Sprite):
    def __init__(self, material, world_x, world_y):
        super().__init__()
        ...
        self.image.fill(material['color']) 
    ...
    def get_material_texture(material):
        texture_name = Texture_file.get(material, {}).get("texture_file")
        color = get_color(material)  # Esta función viene de tu lógica Numerium

        if texture_name:
            return load_and_tint_texture(texture_name, color)
        else:
            return get_cached_surface(color, 255)
...
class WorldGenerator:
    ...
    def generate_chunk_at(self, chunk_x, chunk_y):
        ...
        for x_local in range(CHUNK_SIZE):
            ...
            if (world_block_x % 10 == 0) and (surface_material_name_for_tree_check != "Agua"): 
                ...
                # Utilizamos seed_digits[3] para la altura del tronco
                # Asegúrate de que el índice seed_digits[3] exista
                if len(self.numerium_core.seed_digits) > 3:
                    tree_trunk_height = 2 + (self.numerium_core.seed_digits[3] % 4) 
                else:
                    tree_trunk_height = 3 # Altura por defecto si no hay suficiente dígitos en la semilla
                ...
                # La altura de las hojas se basa en la altura del tronco
                ...
                if wood_material_key_for_properties: # Ensure we found the key for wood
                    # 1. Obtenés el ID numérico desde Numerium
                    material_id = self.numerium_core.get_material_properties(
                        wood_material_key_for_properties,
                        surface_y_block_coord - tree_trunk_height
                    )

                    # 2. Lo convertís en nombre
                    material_name = material_lookup(material_id)

                    # 3. Cargás el diccionario de textura y color
                    material_data = Texture_file.get(material_name, {"color": (100, 100, 100)})
                    print(f"El Material_data_es: ", material_data)
                    # 4. Usás ese material en el bloque
                    wood_block = Block(world_block_x * BLOCK_SIZE, trunk_y * BLOCK_SIZE, material_name)
    ...
    def place_block_at_world_coords(self, world_block_x, world_block_y, material_data):
        ...
        if not self.get_block_at_world_coords(world_block_x, world_block_y): 
            new_block = Block(world_block_x * BLOCK_SIZE, world_block_y * BLOCK_SIZE, material_name)
...
def get_cached_surface(color, alpha):
    # Asegurar que el color sea una tupla RGB válida
    if color is None or not isinstance(color, tuple) or len(color) != 3:
        print("Color inválido detectado, usando magenta de emergencia.")
        color = (255, 0, 255)  # Magenta como marcador de error

    # Asegurar que alpha sea un entero entre 0 y 255
    try:
        alpha = int(alpha)
    ...
    if key not in surface_cache:
        try:
            ...
            print(f"ERROR al crear surface para color={color}, alpha={alpha} → {e}")
            # Devolver surface de error para evitar que se rompa el juego
            fallback_surface = pygame.Surface((BLOCK_SIZE, BLOCK_SIZE), pygame.SRCALPHA)
            fallback_surface.fill((255, 0, 255, 255))  # Magenta total
            return fallback_surface

    return surface_cache[key]
def get_color(material):
    # Ejemplo fijo — reemplazá con tu sistema real
    numerium_colors = {
        "Agua": (0, 150, 255),
        "Madera": (34, 139, 34),
        "Piedra": (80, 80, 80),
    ...
    return numerium_colors.get(material, (255, 0, 255))  # Magenta si no existe
def draw_2d_block(screen, block, camera_offset_x, camera_offset_y, draw_alpha=255):
    ...
    if -BLOCK_SIZE < screen_x < SCREEN_WIDTH and -BLOCK_SIZE < screen_y < SCREEN_HEIGHT:
        ...
        color = get_color(material)  # ⚠️ Esta función la tenés que definir o adaptar
        block_rect = pygame.Rect(screen_x, screen_y, BLOCK_SIZE, BLOCK_SIZE)
        
        texture = get_material_texture(block.material) 
        screen.blit(texture, block_rect)
        
        if texture_name:
            surface = load_and_tint_texture(texture_name, color)
        else:
            surface = get_cached_surface(color, draw_alpha)
...
# --- Inventory System ---
class Inventory:
    def __init__(self, num_slots, block_size):
    ...
    def add_item(self, material_data, count=1):
        ...
        for i, slot in enumerate(self.slots):
            if slot is None:
                self.slots[i] = {'material_data': material_data, 'count': count}
                return True
        return False 
    ...
    def get_selected_item(self):
        """Returns the currently selected material in the inventory."""
        if self.slots[self.selected_slot]:
            return self.slots[self.selected_slot]['material_data']
        return None
    ...
    def draw(self, screen):
        ...
        inventory_x = SCREEN_WIDTH // 2 - (len(self.slots) * (self.block_size + 5)) // 2
...
def initialize_game(randomixer_seed_str, numerium_seed_str, detail_level_str):
    ...
    player = Player(initial_spawn_world_x, initial_spawn_world_y, 
                    ...
                    numerium.jump_force, # numerium.jump_force ahora es la altura deseada en bloques
                    numerium.gravity)
    all_sprites = pygame.sprite.Group(player)

    inventory = Inventory(num_slots=5, block_size=BLOCK_SIZE) 
...
while running:
    ...
    if GAME_STATE == "MENU":
        ...
        text_surface_player = font.render(
            # jump_force ahora es la altura deseada en "metros" conceptuales (bloques)
