...
import random # Importar para generación de números pseudoaleatorios
...
# Define fixed AIR material properties (no longer using BlockType class)
AIR_MATERIAL_PROPERTIES = {
    "name": "Aire",
    "color": (135, 206, 235), # Sky blue for air
    "resistance": 0,
    "structural_resistance": 0,
    "is_fluid": True, # Air is fluid
    "element_dupla_key": "AIR_Fixed_Material_Key",
    "role": 0
}
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, base_strength, jump_force, gravity):
        super().__init__()
        # Player's position in WORLD coordinates (not screen)
        self.world_x = x
        self.world_y = y

        self.image = pygame.Surface([BLOCK_SIZE, BLOCK_SIZE * 2])
        self.image.fill(PLAYER_COLOR)
        # The rect is now initialized at 0,0 because its real position will be handled by the camera
        ...
        # jump_force_factor now represents the desired jump HEIGHT in conceptual "meters" (blocks)
        self.jump_force_factor = jump_force
        self.player_strength = base_strength
        # Ensure gravity has a minimum value to prevent division by zero or physics issues
    ...
    def update(self, world_blocks, world_generator):
        # Creative mode: disable gravity, allow free movement (noclip)
        if game_state["creative_mode"]:
            self.vel_y = 0
            self.on_ground = True # Always on ground for movement

            keys = pygame.key.get_pressed()
            # Vertical movement with W/S keys in creative mode
            if keys[pygame.K_w]:
                self.world_y -= self.move_speed
            if keys[pygame.K_s]:
                self.world_y += self.move_speed

            # Horizontal movement is already handled by start_move_x/stop_move_x
            self.world_x += self.vel_x

            # No collision detection in creative mode
            return # Skip the rest of the update

        # Copy current position for collision detection (Survival Mode)
        ...
        if self.on_ground and self.vel_x != 0 and self.auto_jump_cooldown == 0:
            ...
            for y_offset in range(int(self.jump_force_factor * 2) + 1):
                ...
                # Get world block through world_generator
            ...
            if wall_height > 0 and wall_height <= self.jump_force_factor:
                ...
                if block_at_feet_level_in_front and not block_at_feet_level_in_front.is_fluid():
                    self.jump(world_generator.numerium_core.gravity) # Pass current gravity to jump
                    self.auto_jump_cooldown = self.auto_jump_cooldown_max

        if self.auto_jump_cooldown > 0:
            self.auto_jump_cooldown -= 1

        # --- Horizontal Movement (in world coordinates) ---
        self.world_x += self.vel_x

        # Create a temporary collision rect in world coordinates
        ...
        # --- Gravity and Vertical Movement (in world coordinates) ---
...
class NumeriumCore:
    def __init__(self, randomixer_seed):
        ...
        self.jump_force = self._calculate_jump_force() * 1 # Multiplier for height in blocks/meters
    ...
    def _create_material_properties_map(self):
        ...
        # --- Add DIRT as a new material type, with a dynamically calculated color ---
        # Find the actual keys for Rock and Stone based on their names
        rock_key_for_color_mix = None
        stone_key_for_color_mix = None
        for key, name in self.material_archetypes_by_min_max_key.items():
            if name == "Roca":
                rock_key_for_color_mix = key
            elif name == "Piedra":
                stone_key_for_color_mix = key

        # Get the final colors for Rock and Stone from the already calculated material_name_to_color map
        # Use a fallback color (e.g., a dark grey) if the material isn't found
        rock_color_for_mix = self.material_name_to_color.get("Roca", (80, 80, 80))
        stone_color_for_mix = self.material_name_to_color.get("Piedra", (120, 120, 120))

        # Calculate DIRT color as a mix of Rock and Stone colors
        mixed_base_dirt_color = self._subtractive_mix_rgb(rock_color_for_mix, stone_color_for_mix)

        # Further darken/desaturate the mixed_base_dirt_color to make it more "earthy"
        # Convert to HSV, reduce V and S, then convert back to RGB
        h, s, v = colorsys.rgb_to_hsv(mixed_base_dirt_color[0]/255.0, mixed_base_dirt_color[1]/255.0, mixed_base_dirt_color[2]/255.0)
        dirt_color_final_hsv = colorsys.hsv_to_rgb(h, s * 0.7, v * 0.6) # Reduce saturation to 70%, brightness to 60%
        dirt_color = (int(dirt_color_final_hsv[0]*255), int(dirt_color_final_hsv[1]*255), int(dirt_color_final_hsv[2]*255))

        dirt_base_resistance = 1.0 # Dirt is relatively easy to break
        dirt_is_fluid = False
        material_data_map['Dirt_Derived_Material_Key'] = {
            'name': "Tierra", # Name in Spanish
            'color': dirt_color,
            'base_resistance': dirt_base_resistance,
            'is_fluid': dirt_is_fluid,
            'product': 0 # Dummy product
        }
    ...
    def get_material_properties(self, material_key, y_coord_for_role):
        ...
        SURFACE_WORLD_Y_THRESHOLD = 200 # Blocks above this are "surface"
        MID_DEPTH_WORLD_Y_THRESHOLD = 220 # Blocks above this are "mid-depth"
        DEEP_DEPTH_WORLD_Y_THRESHOLD = 250 # Blocks above this are "deep-depth"


        role_num = 0
        if y_coord_for_role < SURFACE_WORLD_Y_THRESHOLD:
            role_num = 2 # Surface/Air
        elif y_coord_for_role < MID_DEPTH_WORLD_Y_THRESHOLD:
            role_num = 3 # Mid-depth (e.g., dirt/rock transition)
        elif y_coord_for_role < DEEP_DEPTH_WORLD_Y_THRESHOLD:
            role_num = 4 # Deeper (e.g., rock/stone transition)
        else:
            role_num = 5 # Deepest (e.g., stone/bedrock)
        ...
        # Water always has the fluidity role and low resistance
        if base_name == "Agua":
            role_num = 1
            is_fluid = True
            final_resistance = 0.1 # Very low resistance
        # Leaves always have a fixed low resistance, and are not affected by depth for resistance calculation
        elif base_name == "Hojas":
            role_num = 1 # To use role_capacity = 0.1
            is_fluid = False # Not fluid
            final_resistance = 0.1 # Very low resistance
        # Dirt also has a fixed low resistance for consistency with surface materials
        elif base_name == "Tierra":
            final_resistance = 0.5 # A bit more resistant than leaves, less than rock
    ...
    # This method was missing from NumeriumCore, causing the AttributeError
    def get_complex_noise_value(self, x, y, scale=100.0, octaves=4, persistence=0.5, lacunarity=2.0):
        """
        This function attempts to simulate multi-octave noise, crucial for natural terrain.
        It's a very simplified version for demonstration without a dedicated noise library.
        To make it deterministic for a given (x,y), we combine the coordinates with the seed.
        """
        value = 0
        max_amplitude = 0
        frequency = 1 / scale
        ...
        for _ in range(octaves):
            # Combine coordinates with the numerium_seed for deterministic noise per octave
            seed_for_octave = self.randomixer_seed + int(x * frequency) * 12345 + int(y * frequency) * 67890
            local_random = random.Random(seed_for_octave) # Use a local random instance for this octave
            value += local_random.random() * amplitude
...
class WorldGenerator:
    def __init__(self, numerium_core, numerium_seed_val, detail_level):
        ...
        # Key for the Dirt material
        self.dirt_material_key = 'Dirt_Derived_Material_Key'

        # Duplas de referencia para la lógica de terreno
        # Asegurarse de que estas duplas existan en material_archetypes_by_min_max_key
        # O definir valores por defecto si no se encuentran
        self.leaves_ref_dupla_val = 34 # Para la comparación de hojas
        self.stone_ref_dupla_val = 26 # Para la comparación de piedra
        self.rock_ref_dupla_val = 27 # Para la comparación de roca
    ...
    def are_digits_greater(self, num1_val, num2_val):
        """
        Compares two numbers based on the user's custom rule:
        All digits of num1_val must be strictly greater than the corresponding digits of num2_val.
        Pads with leading zeros if lengths differ.
        Example: 91 > 27 = TRUE (9>2 and 1>7 is FALSE, so this example is incorrect based on the rule.
                                 If it means 9>2 AND 1>7, it would be FALSE.
                                 If it means 9>2 OR 1>7, it would be TRUE.
                                 User stated "sólo es mayor si ambos son TRUE" (only greater if both are TRUE),
                                 which implies AND for all digit pairs.
                                 Let's assume the example was meant to be 98 > 27 = TRUE (9>2 and 8>7).
                                 Or 91 > 27 means "first digit of 91 is > first digit of 27, AND second digit of 91 is > second digit of 27".
                                 This is the most direct interpretation of "ambos son TRUE".
                                 So, 91 > 27 is FALSE because 1 is not > 7.
                                 The example "91 > 27 = TRUE" contradicts "19 > 27 = FALSE".
                                 If 91 > 27 is TRUE, then it must be that at least one digit is greater, and none are smaller.
                                 Let's stick to the strict interpretation of "sólo es mayor si ambos son TRUE" for ALL digits.
                                 So for 91 > 27, it's FALSE because 1 is not greater than 7.
                                 For 98 > 27, it's TRUE because 9>2 AND 8>7.
        """
        s_num1 = str(num1_val)
        s_num2 = str(num2_val)
        ...
        if len(s_num1) != len(s_num2):
            return False # Should not happen with zfill, but as a safeguard

        for d1_char, d2_char in zip(s_num1, s_num2):
            if not (int(d1_char) > int(d2_char)): # Check if d1 is strictly greater than d2
                return False
        return True

    def calculate_dupla_density(self, dupla_val):
        """
        Calculates the 'density' of a dupla as the absolute difference of its digits.
        Example: 34 -> |3-4| = 1; 26 -> |2-6| = 4; 27 -> |2-7| = 5.
        """
        s_dupla = str(dupla_val)
        if len(s_dupla) != 2:
            # Handle cases where the number is not a two-digit dupla
            # For simplicity, if it's a single digit, treat it as |digit - 0|
            if len(s_dupla) == 1:
                return abs(int(s_dupla[0]) - 0)
            return 0 # Default for invalid duplas
        return abs(int(s_dupla[0]) - int(s_dupla[1]))
    ...
    def generate_chunk_at(self, chunk_x, chunk_y):
        ...
        # Get material properties for leaves, dirt, rock, stone, and water
        # Y-coord for get_material_properties doesn't matter for fixed resistance, so 0 is fine.
        leaves_props = self.numerium_core.get_material_properties(self.leaves_material_key, 0)
        dirt_props = self.numerium_core.get_material_properties(self.dirt_material_key, 0)
        # Find the actual keys for Rock and Stone based on their names
        rock_key = None
        stone_key = None
        for key, name in self.numerium_core.material_archetypes_by_min_max_key.items():
            if name == "Roca":
                ...
                stone_key = key
        rock_props = self.numerium_core.get_material_properties(rock_key, 0) if rock_key else None
        stone_props = self.numerium_core.get_material_properties(stone_key, 0) if stone_key else None
        water_props = self.numerium_core.get_material_properties(self.water_dupla_key, 0) if self.water_dupla_key else None
        ...
        for x_local in range(CHUNK_SIZE):
            ...
            # --- Tree/Vegetation Generation Logic ---
            ...
            # We use a random factor based on the x-coordinate to make tree placement less uniform.
            tree_random_factor = random.Random(self.numerium_seed + world_block_x * 13).random()
            if (world_block_x % 10 == 0) and (tree_random_factor > 0.7) and (water_props and not water_props['is_fluid']):
                ...
                if len(self.numerium_core.seed_digits) > 3:
                    tree_trunk_height = 2 + (self.numerium_core.seed_digits[3] % 4) # 2 to 5 blocks
                else:
                ...
                if wood_material_key_for_properties: # Ensure we found the key for wood
                    ...
                    leaves_base_y = surface_y_block_coord - tree_trunk_height # Y coordinate for the base of the leaves (just above the top trunk block)

                    # Place central top leaf block
                    if leaves_props and not self.get_block_at_world_coords(world_block_x, leaves_base_y):
                        leaves_block_center_top = Block(world_block_x * BLOCK_SIZE, leaves_base_y * BLOCK_SIZE, leaves_props)
                    ...
                    if leaves_props and not self.get_block_at_world_coords(leaves_block_side_left_x, leaves_base_y):
                        leaves_block_side_left = Block(leaves_block_side_left_x * BLOCK_SIZE, leaves_base_y * BLOCK_SIZE, leaves_props)
                        new_chunk_blocks.add(leaves_block_side_left)
                    if leaves_props and not self.get_block_at_world_coords(leaves_block_side_right_x, leaves_base_y):
                        leaves_block_side_right = Block(leaves_block_side_right_x * BLOCK_SIZE, leaves_base_y * BLOCK_SIZE, leaves_props)
                    ...
                    if leaves_props and not self.get_block_at_world_coords(world_block_x, leaves_block_top_y):
                        leaves_block_top = Block(world_block_x * BLOCK_SIZE, leaves_block_top_y * BLOCK_SIZE, leaves_props)
            ...
            for y_local in range(CHUNK_SIZE):
                ...
                block_to_place_props = None

                # Lógica de capas de terreno:
                if world_block_y == surface_y_block_coord:
                    # Superficie: Hojas (LEAVES)
                    # Usamos un valor de ruido para la comparación con la dupla de hojas (34)
                    density_check_val = int(self.numerium_core.get_complex_noise_value(world_block_x, world_block_y, scale=self.detail_level) * 100)
                    if self.are_digits_greater(density_check_val, self.leaves_ref_dupla_val):
                        block_to_place_props = leaves_props
                    else:
                        block_to_place_props = dirt_props # Default surface if not leaves
                elif world_block_y > surface_y_block_coord: # Debajo de la superficie
                    depth = world_block_y - surface_y_block_coord

                    # Ruido de densidad para la distribución de bloques en capas
                    density_noise_for_layer = self.numerium_core.get_complex_noise_value(
                        world_block_x, world_block_y,
                        scale=self.detail_level * 0.5, # Más suave para las capas
                        octaves=4, persistence=0.5, lacunarity=2.0
                    )
                    density_check_val_layer = int(density_noise_for_layer * 100)

                    # Capa de Tierra (DIRT) justo debajo de la superficie
                    if depth < 3: # Las primeras 2-3 capas son de tierra
                        block_to_place_props = dirt_props
                    # Capa de Roca (ROCK)
                    elif depth < 10: # Entre 3 y 9 bloques de profundidad
                        # Roca si la comparación de dígitos es TRUE con ROCK_REF_DUPLA_VAL o si el ruido es alto
                        if self.are_digits_greater(density_check_val_layer, self.rock_ref_dupla_val) or density_noise_for_layer > 0.6:
                            block_to_place_props = rock_props
                        else:
                            block_to_place_props = dirt_props # Puede haber tierra mezclada
                    # Capa de Piedra (STONE)
                    elif depth < 25: # Entre 10 y 24 bloques de profundidad
                        # Piedra si la comparación de dígitos es TRUE con STONE_REF_DUPLA_VAL o si el ruido es muy alto
                        if self.are_digits_greater(density_check_val_layer, self.stone_ref_dupla_val) or density_noise_for_layer > 0.7:
                            block_to_place_props = stone_props
                        else:
                            block_to_place_props = rock_props # Puede haber roca mezclada
                    else: # Más profundo, principalmente piedra
                        block_to_place_props = stone_props

                    # Lógica para acuíferos de agua entre piedra y roca (simulando medio círculo)
                    # "radio la resta de 4-8 = 4 (sin importar el signo)"
                    # Interpretación: Si la densidad de la capa de roca y piedra es '4' o '5', y el ruido es favorable,
                    # genera agua. La "resta de 4-8 = 4" se usará como un umbral.
                    # Asumimos que 4 y 8 son valores de referencia para la "densidad" del agua.
                    # Densidad de Agua (ejemplo, si su dupla fuera 48): |4-8| = 4
                    water_density_threshold = self.calculate_dupla_density(48) # Usamos 48 como ejemplo para el agua

                    # Check if we are in the transition zone between Rock and Stone
                    is_in_rock_stone_transition = (depth >= 3 and depth < 25)

                    # Ruido para la probabilidad de agua
                    water_noise = self.numerium_core.get_complex_noise_value(
                        world_block_x, world_block_y,
                        scale=self.detail_level * 0.2, # Gran escala para acuíferos
                        octaves=2, persistence=0.5, lacunarity=2.0
                    )

                    # Generar agua si estamos en la zona de transición y el ruido es favorable
                    # y la "densidad" del agua es relevante para la posición.
                    # Para simular "medio círculo", el agua será más probable en el centro de la columna
                    # de agua y disminuirá hacia los lados.
                    # Usaremos una probabilidad basada en la distancia al "centro" del acuífero.
                    # Para un "medio círculo", podemos usar una función de coseno o una parábola.
                    # Simplificaremos con un umbral de ruido y la "densidad" del agua.

                    if is_in_rock_stone_transition and water_props and water_noise > 0.7: # Ajustar umbral de ruido para agua
                        # La probabilidad de agua es mayor si la densidad del acuífero es cercana al umbral
                        # y si el "valor de comparación" del ruido es "mayor" que la densidad del agua.
                        noise_for_water_comparison = int(water_noise * 100)
                        if self.are_digits_greater(noise_for_water_comparison, water_density_threshold):
                            block_to_place_props = water_props
                            # Para simular "medio círculo", podemos hacer que el agua sea menos probable
                            # en los bordes de la "zona de acuífero" de la columna.
                            # Esto es una simplificación, un medio círculo real requeriría más cálculo de forma.
                            # Por ahora, se generará como una capa horizontal con probabilidad.


                # Place the determined block
                if block_to_place_props:
                    # Ensure we don't overwrite existing tree blocks if they were placed
                    if not self.get_block_at_world_coords(world_block_x, world_block_y):
                        block = Block(world_block_x * BLOCK_SIZE, world_block_y * BLOCK_SIZE, block_to_place_props)
...
class Inventory:
    def __init__(self, num_slots_hotbar, num_rows_full_inventory, num_cols_full_inventory, block_size):
        ...
        self.dragged_item_original_slot = None # ('hotbar', index) or ('full_inventory', row, col)
        self.drag_offset_x, self.drag_offset_y = 0, 0 # Offset from mouse to item top-left corner

    def add_item(self, material_data, count=1):
        """Adds a material to the inventory, stacking it if it already exists."""
        # In creative mode, we always want to add a full stack for visual representation
        # and not worry about actual count for consumption.
        if game_state["creative_mode"]:
            count = 64 # Always add a full stack in creative mode
        ...
        for i, slot in enumerate(self.hotbar_slots):
            if slot and slot['material_data']['name'] == material_data['name']:
                slot['count'] = count # Set to max stack in creative, or add in survival
                return True
        for i, slot in enumerate(self.hotbar_slots):
            if slot is None:
    ...
    def remove_item(self, material_data_name, count=1):
        ...
        if game_state["creative_mode"]:
            return True # In creative mode, items are not consumed
    ...
    def get_current_tool_strength(self):
        ...
        for tool_type in tool_material_types:
            for slot in self.hotbar_slots:
                if slot and slot['material_data'] and tool_type in slot['material_data']['name']:
    ...
    def _get_hotbar_slot_rect(self, index):
        """Helper to get the rect of a hotbar slot."""
        inventory_x = SCREEN_WIDTH // 2 - (len(self.hotbar_slots) * (self.block_size + 5)) // 2
        inventory_y = SCREEN_HEIGHT - (self.block_size + 10)
        return pygame.Rect(inventory_x + index * (self.block_size + 5), inventory_y, self.block_size, self.block_size)

    def _get_full_inventory_slot_rect(self, r, c):
        """Helper to get the rect of a full inventory slot."""
        grid_width = self.num_cols_full * (self.block_size + 10) - 10
        grid_height = self.num_rows_full * (self.block_size + 10) - 10
        start_x = (SCREEN_WIDTH - grid_width) // 2
        start_y = (SCREEN_HEIGHT - grid_height) // 2
        hotbar_display_y = start_y + 50
        main_inventory_start_y = hotbar_display_y + self.block_size + 20
        return pygame.Rect(start_x + c * (self.block_size + 10), main_inventory_start_y + r * (self.block_size + 10), self.block_size, self.block_size)

    def _place_item_in_slot(self, slot_coords, item_data):
        """Places an item_data dictionary into the specified slot."""
        slot_type = slot_coords[0]
        if slot_type == 'hotbar':
            index = slot_coords[1]
            self.hotbar_slots[index] = item_data
        elif slot_type == 'full_inventory':
            row, col = slot_coords[1], slot_coords[2]
            self.full_inventory_slots[row][col] = item_data

    def handle_event(self, event, mouse_pos):
        """Handles keyboard and mouse events for the inventory."""
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1: # Left click for dragging
                # If currently dragging, this click means dropping
                if self.is_dragging:
                    return # Handled by MOUSEBUTTONUP

                # Check hotbar slots for starting drag
                for i in range(len(self.hotbar_slots)):
                    slot_rect = self._get_hotbar_slot_rect(i)
                    slot_data = self.hotbar_slots[i]
                    if slot_rect.collidepoint(mouse_pos) and slot_data:
                        self.is_dragging = True
                        self.dragged_item_data = slot_data # Store the full slot data (material_data and count)
                        self.dragged_item_original_slot = ('hotbar', i)
                        self.drag_offset_x = slot_rect.x - mouse_pos[0]
                        self.drag_offset_y = slot_rect.y - mouse_pos[1]
                        # Temporarily remove item from original slot for visual feedback
                        self.hotbar_slots[i] = None
                        return
                # Check full inventory slots for starting drag (only if inventory is open)
                if game_state["current_game_state"] == "INVENTORY":
                    for r in range(self.num_rows_full):
                        for c in range(self.num_cols_full):
                            slot_rect = self._get_full_inventory_slot_rect(r, c)
                            slot_data = self.full_inventory_slots[r][c]
                            if slot_rect.collidepoint(mouse_pos) and slot_data:
                                self.is_dragging = True
                                self.dragged_item_data = slot_data # Store the full slot data
                                self.dragged_item_original_slot = ('full_inventory', r, c)
                                self.drag_offset_x = slot_rect.x - mouse_pos[0]
                                self.drag_offset_y = slot_rect.y - mouse_pos[1]
                                # Temporarily remove item from original slot
                                self.full_inventory_slots[r][c] = None
                                return

        elif event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1 and self.is_dragging:
                target_slot_found = False
                
                # Try to place in hotbar slots
                for i in range(len(self.hotbar_slots)):
                    slot_rect = self._get_hotbar_slot_rect(i)
                    if slot_rect.collidepoint(mouse_pos):
                        # If target slot has an item, swap them
                        if self.hotbar_slots[i]:
                            temp_item = self.hotbar_slots[i]
                            self.hotbar_slots[i] = self.dragged_item_data
                            self._place_item_in_slot(self.dragged_item_original_slot, temp_item)
                        else: # Place in empty slot
                            self.hotbar_slots[i] = self.dragged_item_data
                        target_slot_found = True
                        break

                # Try to place in full inventory slots (only if inventory is open)
                if not target_slot_found and game_state["current_game_state"] == "INVENTORY":
                    for r in range(self.num_rows_full):
                        for c in range(self.num_cols_full):
                            slot_rect = self._get_full_inventory_slot_rect(r, c)
                            if slot_rect.collidepoint(mouse_pos):
                                # If target slot has an item, swap them
                                if self.full_inventory_slots[r][c]:
                                    temp_item = self.full_inventory_slots[r][c]
                                    self.full_inventory_slots[r][c] = self.dragged_item_data
                                    self._place_item_in_slot(self.dragged_item_original_slot, temp_item)
                                else: # Place in empty slot
                                    self.full_inventory_slots[r][c] = self.dragged_item_data
                                target_slot_found = True
                ...
                if not target_slot_found:
                    # If no valid target, return to original slot
                    self._place_item_in_slot(self.dragged_item_original_slot, self.dragged_item_data)

                # Reset dragging state
                self.is_dragging = False
                self.dragged_item_data = None
                self.dragged_item_original_slot = None
    ...
    def draw_hotbar(self, screen):
        ...
        # Draw the dragged item if currently dragging from hotbar and not in full inventory view
        if self.is_dragging and self.dragged_item_data and self.dragged_item_original_slot and self.dragged_item_original_slot[0] == 'hotbar' and game_state["current_game_state"] == "GAME":
            mouse_pos = pygame.mouse.get_pos()
            item_x = mouse_pos[0] + self.drag_offset_x
            item_y = mouse_pos[1] + self.drag_offset_y
            block_image = pygame.Surface([self.block_size - 4, self.block_size - 4])
            block_image.fill(self.dragged_item_data['material_data']['color'])
            screen.blit(block_image, (item_x + 2, item_y + 2))
            # Also draw count
            count_text = self.font.render(str(self.dragged_item_data['count']), True, WHITE)
            screen.blit(count_text, (item_x + self.block_size - count_text.get_width() - 2, item_y + self.block_size - count_text.get_height() - 2))
    ...
    def draw_inventory_grid(self, screen):
        ...
        # Draw the dragged item if any (only when full inventory is open)
        if self.is_dragging and self.dragged_item_data:
            mouse_pos = pygame.mouse.get_pos()
            item_x = mouse_pos[0] + self.drag_offset_x
            item_y = mouse_pos[1] + self.drag_offset_y
            block_image = pygame.Surface([self.block_size - 4, self.block_size - 4])
            block_image.fill(self.dragged_item_data['material_data']['color']) # Use material_data['color']
            screen.blit(block_image, (item_x + 2, item_y + 2))
            # Also draw count
            count_text = self.font.render(str(self.dragged_item_data['count']), True, WHITE)
            screen.blit(count_text, (item_x + self.block_size - count_text.get_width() - 2, item_y + self.block_size - count_text.get_height() - 2))
...
# --- Game State ---
game_state = {
    "current_seed": 12345, # Default seed (not directly used for generation anymore, but for display)
    "inventory": {},
    "selected_inventory_slot": 0,
    "creative_mode": False, # New: Creative Mode flag
    "current_game_state": "MENU" # Use a single state variable
}
...
def initialize_game(randomixer_seed_str, numerium_seed_str, detail_level_str):
    ...
    # Add initial items to inventory for testing
    # Get material properties using the numerium core
    stone_key = None
    wood_key = None
    rock_key = None
    water_key = None

    for key, name in numerium.material_archetypes_by_min_max_key.items():
        if name == "Piedra":
            stone_key = key
        elif name == "Madera":
            wood_key = key
    ...
    # Add items to inventory, using the correct material properties
    # IMPORTANT: Pass the material properties obtained from get_material_properties()
    # This ensures the 'resistance' key is present and all other derived properties.
    # The y_coord_for_role is set to 0 as these are generic inventory items, not tied to a specific world depth.
    if numerium.material_properties_map.get('Dirt_Derived_Material_Key'):
        inventory.add_item(numerium.get_material_properties('Dirt_Derived_Material_Key', 0), 64)
    if stone_key and numerium.material_properties_map.get(stone_key):
        inventory.add_item(numerium.get_material_properties(stone_key, 0), 64)
    if wood_key and numerium.material_properties_map.get(wood_key):
        inventory.add_item(numerium.get_material_properties(wood_key, 0), 64)
    if numerium.material_properties_map.get('Hojas_Derived_Material_Key'):
        inventory.add_item(numerium.get_material_properties('Hojas_Derived_Material_Key', 0), 64)
    if rock_key and numerium.material_properties_map.get(rock_key):
        inventory.add_item(numerium.get_material_properties(rock_key, 0), 64)
    if water_key and numerium.material_properties_map.get(water_key):
        inventory.add_item(numerium.get_material_properties(water_key, 0), 64)
...
while running:
    for event in pygame.event.get():
        ...
        if game_state["current_game_state"] == "MENU":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if start_button_rect.collidepoint(event.pos):
                    ...
                    if initialize_game(randomixer_seed_input.get_text(), numerium_seed_input.get_text(), detail_level_input.get_text()):
                        game_state["current_game_state"] = "GAME"

        elif game_state["current_game_state"] == "GAME":
            player.update_strength_from_inventory(inventory)

            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    player.jump(numerium.gravity) # Pass current gravity to jump
                if event.key == pygame.K_a:
                    player.start_move_x(-1)
                if event.key == pygame.K_d:
                    player.start_move_x(1)
                if event.key == pygame.K_e: # Toggle inventory
                    game_state["current_game_state"] = "INVENTORY"
                if event.key == pygame.K_c: # Toggle creative mode
                    game_state["creative_mode"] = not game_state["creative_mode"]
                    print(f"Modo creativo: {'ACTIVADO' if game_state['creative_mode'] else 'DESACTIVADO'}")
            ...
            # Handle inventory drag/drop events in GAME state (for hotbar)
            inventory.handle_event(event, pygame.mouse.get_pos())
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if event.button == 1: # Left Click (Mine)
                    ...
                    if block_to_mine:
                        # Creative mode: instantly break block
                        if game_state["creative_mode"]:
                            ...
                            if mined_block_data:
                                ...
                                if inventory.add_item(mined_block_data.material_data, count=64): # Add a stack in creative
                                    print(f"¡{mined_block_data.get_name()} minado y añadido al inventario (creativo)!")
                                else:
                                    print("Inventario lleno. Bloque minado pero no añadido (creativo).")
                            else:
                    ...
                    if selected_item:
                        # Creative mode: place block without consuming from inventory
                        if game_state["creative_mode"]:
                            if not world_generator.get_block_at_world_coords(block_x_coord_clicked, block_y_coord_clicked):
                                ...
                                if placed_block:
                                    active_blocks_group.add(placed_block)
                                    print(f"Bloque {selected_item['name']} colocado en ({block_x_coord_clicked},{block_y_coord_clicked}) (creativo)!")
                                else:
        ...
        elif game_state["current_game_state"] == "INVENTORY":
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_e: # Toggle inventory off
                    game_state["current_game_state"] = "GAME"
            
            # Handle inventory drag/drop events in INVENTORY state
            inventory.handle_event(event, pygame.mouse.get_pos())
    ...
    if game_state["current_game_state"] == "MENU":
    ...
    elif game_state["current_game_state"] == "GAME" or game_state["current_game_state"] == "INVENTORY":
        # Update player's world position only if in GAME state
        if game_state["current_game_state"] == "GAME":
        ...
        # Display creative mode status
        creative_mode_text = font.render(f"Modo Creativo: {'ON' if game_state['creative_mode'] else 'OFF'}", True, WHITE)
        screen.blit(creative_mode_text, (10, 100))
