...
traductor = {} 
respuesta_anterior = ""  # Variable global para guardar última respuesta
...
def proyectar_interes_36():
    """
    Proyecta una respuesta de 'longitud' palabras.
    Por defecto: 10 palabras (la original)
    Si quieres más largo: 15, 20, 30...
    """
    ...
    # Selecciona TODAS las candidatas, no solo 10
...
# =====================================================================
# NUEVA FUNCIÓN: AJUSTE AUTOMÁTICO DE LONGITUD
# =====================================================================

def ajustar_longitud_respuesta():
    """
    Bella analiza su estado interno y decide cuánto debe extenderse.
    Si está relajada → respuestas más largas y creativas.
    Si está estresada → respuestas más cortas y concisas.
    """
    global cantidad_palabras
    
    # 1. Calcular estado interno
    ...
    if total_activas == 0:
        ...
        promedio_presion = presion_total / total_activas
    
    # 3. Decidir longitud según el estado
    #    - Presión baja (< 20) → relajada → respuesta larga (15-25 palabras)
    #    - Presión media (20-50) → equilibrada → respuesta media (10-15)
    #    - Presión alta (> 50) → estresada → respuesta corta (5-10)
    
    if promedio_presion < 20:
        nueva_longitud = 20
        estado = "relajada"
    elif promedio_presion < 40:
        nueva_longitud = 15
        estado = "equilibrada"
    elif promedio_presion < 60:
        nueva_longitud = 10
        estado = "ligeramente estresada"
    else:
        nueva_longitud = 6
        estado = "estresada"
    
    # 4. También considerar la cantidad de neuronas activas
    #    Más neuronas → más palabras disponibles → puede ser más larga
    if total_activas > 300:
        nueva_longitud = min(nueva_longitud + 5, 30)
    
    # 5. Guardar y mostrar
    cantidad_palabras = nueva_longitud
    print(f"\n📊 Bella se ajusta automáticamente:")
    print(f"   Estado interno: {estado}")
    print(f"   Presión promedio: {promedio_presion:.1f}")
    print(f"   Neuronas activas: {total_activas}")
    print(f"   Longitud de respuesta: {cantidad_palabras} palabras")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("[7] Evaluar")
        print(f"[8] Configurar longitud de respuesta (actual {cantidad_palabras})")
        print("[9] Ajuste automático de longitud de respuesta")
        #opciones
        ...
        # En el menú principal, agregar:
            ...
            if not respuesta_anterior:
                print("❌ No hay respuesta anterior que evaluar.")
                continue
            
            calidad = input("¿Fue buena la respuesta? (good/bad): ").strip().lower()
            
            if calidad == "good":
                print(f"✅ Reforzando respuesta: '{respuesta_anterior}'")
                for palabra in respuesta_anterior.split():
                    if palabra:  # Evitar palabras vacías
                        idx = xorid(palabra) & MASK_NEURONAS  # <--- ¡MÁSCARA AQUÍ!
                        base = (idx << 6) + (idx << 1) + idx
                        incremento = 100
                        exps_flat[base + 2] = min(MAX_UINT32, int(exps_flat[base + 2]) + incremento)
                        cerebro_asm.mente[idx] += incremento
                        
            elif calidad == "bad":
                print(f"❌ Debilitando respuesta: '{respuesta_anterior}'")
                for palabra in respuesta_anterior.split():
                    if palabra:
                        idx = xorid(palabra) & MASK_NEURONAS  # <--- ¡MÁSCARA AQUÍ!
                        base = (idx << 6) + (idx << 1) + idx
                        decremento = 50
                        exps_flat[base + 2] = max(0, int(exps_flat[base + 2]) - decremento)
                        cerebro_asm.mente[idx] = max(0, cerebro_asm.mente[idx] - decremento)
            else:
                print("⚠️ Respuesta no válida. Usa 'good' o 'bad'.")
                continue
            
            print(f"💭 BELLA recordará esto: {calidad}")
        elif opcion == "8":
            try:
                nueva_longitud = int(input("Longitud deseada (5-500): "))
                cantidad_palab = max(5, min(500, nueva_longitud))
                print(f"✅ Longitud de respuesta configurada a {longitud_respuesta} palabras.")
            except ValueError:
                print("⚠️ Usa un número entero.")
        elif opcion == "9":
            print("\n--- AJUSTE AUTOMÁTICO ---")
            ajustar_longitud_respuesta()
            print("\n💡 Puedes llamar a esta opción cuando quieras que Bella se reajuste.")
